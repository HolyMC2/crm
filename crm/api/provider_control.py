...
OUTCOMES = {"ours", "other_app", "not_owner", "unsupported", "unconfigured", "account_invalid",
            "account_changed", "denied", "rate_limited", "invalid", "unavailable"}
OWNER_KINDS = {None, "this_app", "meta_inbox", "other_app"}
_UNAVAILABLE = ("unavailable", "provider_control_unavailable",
                "No se pudo confirmar con Meta ahora. Vuelve a verificar en unos minutos; "
                "mientras tanto responde desde la bandeja de Meta.")
_NO_ADAPTER = ("unsupported", "provider_control_unsupported",
...
def _inconclusive(outcome, reason, guidance):
    return {"outcome": outcome, "provider_control": None, "reason_code": reason, "owner_kind": None,
            "owner_app_id": None, "expiration": None, "app_id": None, "account_fingerprint": None,
            "checked_at": str(now_datetime()), "guidance": guidance}
...
def _clean(result):
    ...
    values = {key: result.get(key) for key in ("provider_control", "reason_code", "owner_kind", "owner_app_id",
                                               "expiration", "app_id", "account_fingerprint", "checked_at", "guidance")}
    digits = lambda value: value is None or (isinstance(value, str) and re.fullmatch(r"[0-9]{1,32}", value))
    valid = (values["provider_control"] == PROVES.get(outcome)
        and isinstance(values["reason_code"], str) and re.fullmatch(r"[a-z0-9_]{1,100}", values["reason_code"])
        and values["owner_kind"] in OWNER_KINDS and digits(values["owner_app_id"]) and digits(values["app_id"])
        and (values["expiration"] is None or (type(values["expiration"]) is int and 0 <= values["expiration"] < 10 ** 13))
        and (values["account_fingerprint"] is None
             or (isinstance(values["account_fingerprint"], str) and re.fullmatch(r"[0-9a-f]{64}", values["account_fingerprint"])))
        and isinstance(values["checked_at"], str) and 0 < len(values["checked_at"]) <= 40
        and isinstance(values["guidance"], str) and 0 < len(values["guidance"]) <= 600
...
def _query(doc):
    ...
    scope = frappe._dict(name=doc.name, provider=doc.provider, account_id=doc.account_id,
                         peer_id=doc.peer_id, account_record=doc.account_record)
...
def _notify_owner(doc, actor):
    ...
    frappe.publish_realtime("crm_conversation_updated", {"name": doc.name, "generation": doc.generation},
                            user=owner, after_commit=True)


@frappe.whitelist(methods=["POST"])
def verify(name, expected_generation, command_id):
    ...
    with control.conversation_fence(name):
        ...
        public = {key_: verification[key_] for key_ in ("outcome", "reason_code", "owner_kind", "owner_app_id",
                                                         "expiration", "app_id", "account_fingerprint", "checked_at")}
        public.update(provider_control_target=verification["provider_control"],
                      send_ready=doc.provider_control == "Ours")
        result = control._persist_transition(doc, before, key=key, fingerprint=fingerprint, origin="Human",
            actor=actor, action="provider_verify", reason=verification["reason_code"],
