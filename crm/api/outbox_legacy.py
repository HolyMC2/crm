...
def guard_legacy_send(provider, account_id, peer_id, *, payload):
	...
	with ExitStack() as stack:
		try:
			...
			if not all(
				frappe.db.exists("DocType", name) for name in (control.DOCTYPE, control.EVENT, outbox.DOCTYPE)
			):
			...
			row = frappe.db.get_value(
				control.DOCTYPE,
				name,
				["name", "provider", "account_id", "peer_id"],
				as_dict=True,
				for_update=True,
			)
			if row:
				...
				if (
					not isinstance(grant, tuple)
					or len(grant) != 6
					or grant[1:4] != (provider, account_id, peer_id)
				):
