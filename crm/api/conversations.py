...
PUBLIC_FIELDS = (
    ...
    "bot_enabled", "provider_control", "modified", "department", "automation_state",
)
...
def internal_webchat_customer_reply(conversation_name, message_key):
    ...
    with conversation_fence(conversation_name):
        ...
        message = frappe.db.get_value("CRM Webchat Message", message_key,
            ["name", "channel", "session", "conversation", "direction", "text", "text_hash", "control_generation", "creation"],
        ...
        if replay:
            return replay
        evidence = {"kind": "webchat", "provider": doc.provider, "account_id": doc.account_id, "peer_id": doc.peer_id,
                    "message": message.name, "text": message.text, "received_at": str(message.creation),
        ...
        result = _persist_transition(doc, before, key=key, fingerprint=fingerprint,
            origin="System", actor=None, action="customer_reply", reason=reason, grant=grant)
...
def _claimed_receipt(receipt_name, provider, account_id):
    if getattr(frappe.local, "request", None) is not None or not receipt_name \
            or receipt_name != frappe.flags.get("meta_webhook_receipt"):
        _deny()
    row = frappe.db.get_value("Meta Webhook Receipt", receipt_name,
        ["provider", "account_id", "event_type", "state", "attempts", "lease_until"], as_dict=True, for_update=True)
    if not row or row.state != "Processing" or row.provider != provider or row.account_id != account_id \
            or not row.attempts or not row.lease_until or frappe.utils.get_datetime(row.lease_until) <= now_datetime():
        _deny()
...
def internal_customer_message(provider, account_id, peer_id, *, receipt_name, text=None, received_at=None,
                              continuation=True):
    ...
    evidence = {"kind": "meta", "provider": provider, "account_id": account_id, "peer_id": peer_id,
                "message": receipt_name, "text": text[:4096], "received_at": str(received_at or now_datetime()),
                "generation": None}
    with conversation_fence(name):
        from crm.conversation_scope import assert_customer_peer
        assert_customer_peer(provider, peer_id)
        if not frappe.db.get_value(DOCTYPE, name, "name", for_update=True):
            policy_exists = frappe.db.exists("DocType", "CRM Automation Policy") and frappe.db.get_value(
                "CRM Automation Policy", {"provider": provider, "account_id": account_id, "status": "Published"}, "name")
        ...
        if continuation and doc.control_state == "Bot" and doc.bot_enabled:
            reason, grant = customer_reply_control(doc, evidence)
        result = _persist_transition(doc, before, key=key, fingerprint=fingerprint, origin="Provider", actor=None,
            action="customer_reply", reason=reason, receipt=receipt_name, grant=grant)
        if before["control_state"] != "Bot":
...
def _customer_input(doc, evidence):
    ...
    try:
        ...
        if not isinstance(result, dict) or result.get("accepted") is not True:
            ...
            code = code if isinstance(code, str) and re.fullmatch(r"[a-z0-9_]{1,80}", code) else "customer_input_unmatched"
            return False, code, {"department": department} if department in departments.customer_ids() else None
...
def _policy_start(doc, evidence, command_id):
    ...
    try:
        ...
        frappe.db.rollback(save_point=point)
        frappe.local.message_log = messages
        frappe.log_error(title="CRM automation policy start refused", message=f"policy={policy.name} conversation={doc.name}")
        return None
...
def _verify_ingress(doc, command_id, generation):
    ...
    if not command_id.startswith("inbound:") or not evidence:
        _deny()
    if kind == "webchat":
        row = frappe.db.get_value("CRM Webchat Message", evidence,
            ["conversation", "direction", "channel", "control_generation"], as_dict=True, for_update=True)
        if not row or doc.provider != "Webchat" or row.direction != "Incoming" or row.conversation != doc.name \
                or row.channel != doc.account_id or row.control_generation != generation:
            _deny()
        return
    if kind == "meta":
...
def begin_policy_bot(name, expected_generation, command_id, *, run_name, policy_name):
    ...
    with conversation_fence(name):
        ...
        if doc.control_state != "Human" or doc.human_owner or doc.provider_control not in {"Ours", "Not Applicable"}:
            _deny()
        policy = policies.published_for(doc.provider, doc.account_id)
        if not policy or policy.name != policy_name or policies.grant_reason(policy, doc) or policies.cooldown_reason(policy, doc):
            _deny()
        _automation_allowed(doc.provider)
        run = frappe.db.get_value("Chatflow Run", run_name, ["flow", "automation_policy", "source_hash"], as_dict=True, for_update=True)
        if not run or run.flow != policy.flow or run.automation_policy != policy.name or run.source_hash != policy.flow_hash:
            _deny()
        snapshot_hash, run_actor = _native_run(run_name, doc, expected_generation, policy.published_by, starting=True)
        before = _snapshot(doc)
        doc.control_state, doc.human_owner, doc.bot_enabled = "Bot", None, 1
        return _persist_transition(doc, before, key=key, fingerprint=fingerprint, origin="System",
            actor=policy.published_by, action="policy_start_bot", reason="published_inbound_policy",
            notify_actor=False, grant={
                "bot_run": run_name, "bot_snapshot_hash": snapshot_hash, "bot_actor_user": run_actor,
                "bot_requested_by": policy.published_by, "bot_policy": policy.name,
...
def _assert_bot_grant(doc, actor, run_name):
    ...
    events = frappe.db.get_values(EVENT, {"conversation": doc.name, "to_generation": doc.generation,
        "action": ["in", ["start_bot", "policy_start_bot"]]}, ["result_json", "actor_user", "action", "origin"],
        as_dict=True, for_update=True)
    if len(events) != 1:
        _deny()
    event = events[0]
    ...
    if event.action == "start_bot":
        ...
        if event.origin != "System" or not isinstance(name, str) or not frappe.db.exists(policies.DOCTYPE, name):
            _deny()
        policy = frappe.get_doc(policies.DOCTYPE, name, for_update=True)
        if policy.published_hash != grant.get("bot_policy_hash") or policy.published_by != event.actor_user \
...
def handoff_bot(name, generation, run_name, command_id, *, actor_user, step_id, owner=None, department=None,
                finished=False):
    ...
    fingerprint = _digest(["bot_handoff", generation, run_name, step_id, owner] + ([department, bool(finished)] if department or finished else []))
    with conversation_fence(name):
        doc = _load(name)
        ...
        if validate_native_handoff(run_name, name, generation, actor_user, step_id, owner, **extra) is not True:
            _deny()
        if department:
        ...
        return _persist_transition(doc, before, key=key, fingerprint=fingerprint,
            origin="System", actor=actor_user, action="bot_handoff",
    ...
    with conversation_fence(name):
        ...
        if action == "request":
            ...
            if doc.control_state != "Closed" or (current_owner != actor and not (manager and reason)):
                _deny()
            doc.control_state, doc.human_owner, doc.bot_enabled = "Human", None, 0
        if before["control_state"] == "Bot" and doc.control_state != "Bot" and frappe.db.has_column(DOCTYPE, "automation_state"):
...
def route(name, department, expected_generation, command_id, reason=None):
    ...
    with conversation_fence(name):
        ...
        return _persist_transition(doc, before, key=key, fingerprint=fingerprint, origin="Human", actor=actor,
            action="route", reason=reason, grant={"previous_department": previous})


def _linkable(doctype, docname, user):
    if not isinstance(doctype, str) or doctype not in departments.record_doctypes() or not frappe.db.exists("DocType", doctype):
...
def _owning_department(doctype, preferred=None):
    if preferred and doctype in departments.record_doctypes(preferred):
        return preferred
    return next((key for key in departments.customer_ids() if doctype in departments.record_doctypes(key)), None)
...
def link_record(name, doctype, docname, command_id, remove=False):
    ...
    with conversation_fence(name):
        ...
        if remove:
            ...
            links.append({"doctype": record.doctype, "name": record.name,
                          "department": _owning_department(record.doctype, doc.get("department")),
                          "added_by": actor, "added_at": str(now_datetime()), "source": "person"})
        before = _snapshot(doc)
        _write_metadata(doc, {"context_links": json.dumps(links, sort_keys=True, separators=(",", ":"))})
        return _persist_transition(doc, before, key=key, fingerprint=fingerprint, origin="Human", actor=actor,
            action="unlink" if remove else "link", reason=f"{doctype}:{docname}"[:500])
...
def internal_automation_link(name, generation, run_name, *, actor_user, doctype, docname):
    ...
    with conversation_fence(name):
        ...
        links.append({"doctype": record.doctype, "name": record.name,
                      "department": _owning_department(record.doctype, doc.get("department")),
                      "added_by": actor_user, "added_at": str(now_datetime()), "source": "automation:" + run_name[:100]})
        _write_metadata(doc, {"context_links": json.dumps(links, sort_keys=True, separators=(",", ":"))})
        return links
...
def internal_automation_state(name, run_name, state):
    ...
    if not all(frappe.db.has_column(DOCTYPE, field) for field in ("automation_run", "automation_state")):
        return
    _assert_fence(name)
    frappe.db.set_value(DOCTYPE, name, {"automation_run": _text(run_name), "automation_state": state}, update_modified=False)
...
def context_view(doc, user=None):
    ...
    for link in departments.parse_links(doc.get("context_links")):
        ...
        out.append({"doctype": doctype, "name": docname, "label": str(label or docname)[:140],
                    "department": link.get("department"), "source": (link.get("source") or "person").split(":")[0],
