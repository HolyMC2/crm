...
DEPARTMENTS = {
	"marketing": {
		"label": "Marketing",
		"customer": True,
		"members": ("Marketing Manager", "Marketing User"),
	...
	"support": {
		"label": "Support",
		"customer": True,
		"members": ("Support Team", "Agent", "Agent Manager"),
	...
	"finance": {
		"label": "Finance",
		"customer": True,
		"members": ("Accounts User", "Accounts Manager"),
		"managers": ("Accounts Manager",),
		"records": ("Sales Invoice", "POS Invoice", "Payment Entry", "Payment Request"),
	},
	"hr": {
		"label": "HR",
		"customer": False,
		"members": ("HR User", "HR Manager"),
		"managers": ("HR Manager",),
		"records": (),
	},
	"warehouse": {
		"label": "Warehouse",
		"customer": True,
		"members": ("Stock User", "Stock Manager"),
		"managers": ("Stock Manager",),
...
def member_departments(roles, *, customer=True):
	return [
		key for key in DEPARTMENTS if is_member(roles, key) and (DEPARTMENTS[key]["customer"] or not customer)
	]
		...
		out.append(
			{
				...
				"member": is_member(roles, key),
				"manager": is_manager(roles, key),
...
def parse_links(raw):
	...
	for item in value[:MAX_LINKS]:
		if (
			...
			and isinstance(item.get("doctype"), str)
			and isinstance(item.get("name"), str)
		):
			out.append(
				{
					key: item.get(key)
					for key in ("doctype", "name", "department", "added_by", "added_at", "source")
				}
			)
	return out
