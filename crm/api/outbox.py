...
TRANSITIONS = {
	...
	"Failed": {"Queued", "Cancelled", "Delivered", "Read"},
	"Cancelled": set(),
	"Unknown": set(),
}
...
def _projection(doc):
	...
	if doc.provider == "WhatsApp":
		...
		result["text"] = ""
	result["can_retry"] = (
		doc.state in {"Blocked", "Failed", "Deferred"}
		and not doc.provider_message_id
		and doc.attempts < MAX_ATTEMPTS
	)
	result["can_cancel"] = (
		doc.state in {"Queued", "Claimed", "Blocked", "Deferred", "Failed"} and not doc.provider_message_id
	)
	return result
...
def _notify(doc):
	frappe.publish_realtime(
		"crm_outbox_updated",
		{"name": doc.name, "conversation": doc.conversation},
		user=doc.actor_user,
		after_commit=True,
	)
...
def _enqueue(names):
	for name in names:
		try:
			frappe.enqueue(
				"crm.api.outbox.dispatch_intent",
				intent_name=name,
				queue="short",
				timeout=90,
				job_id="crm-outbox-" + name,
				deduplicate=True,
			)
		except Exception:
...
def _pending_on_queue_contention(function):
	...
	def protected(*args, **kwargs):
		try:
			...
			raise ReplyRequestPending(
				"Request could not be confirmed. Check the same request again."
			) from None
	...
	with control.conversation_fence(conversation):
		...
		if current.provider in HOOKED_CHANNELS:
			...
			fields = (
				...
				if current.provider == "Webchat"
				else ["name", "app_id", "business_id"]
			)
			account = frappe.db.get_value(
				source, current.account_record, fields, as_dict=True, for_update=True
			)
			source_name = account.name
			revision = (
				webchat_revision(account) if current.provider == "Webchat" else account_revision(account)
			)
		doc = frappe.get_doc(
			{
				"doctype": DOCTYPE,
...
def _list_intents(conversation, limit, before):
	...
	rows = frappe.db.sql(
		"SELECT name FROM `tabCRM Outbound Intent` WHERE conversation=%s"
		+ clause
		+ " ORDER BY creation DESC,name DESC LIMIT %s",
		(*params, limit),
		as_dict=True,
	)
...
def _eligibility(doc):
	...
	try:
		current = control.assert_current_generation(
			doc.conversation,
			doc.conversation_generation,
			origin=doc.origin,
			actor_user=doc.actor_user,
			run_name=doc.run_name,
		)
		if (doc.provider, doc.account_id, doc.peer_id) != (
			current.provider,
			current.account_id,
			current.peer_id,
		):
...
def _gateway(doc):
	...
	payload = json.loads(doc.payload)
	token = _dispatch.set(
		(doc.name, doc.provider, doc.account_id, doc.peer_id, doc.payload.encode(), doc.claim_token)
	)
	try:
	...
	if state == "Accepted":
		...
		if doc.provider in HOOKED_CHANNELS:
			valid = (
				...
				and bool(_META_MESSAGE_ID.fullmatch(message_id))
			...
			valid = (
				isinstance(message_id, str)
				and bool(re.fullmatch(pattern, message_id))
...
def _accept_provider_id(doc, message_id):
	...
	try:
		...
		if not frappe.db.get_value(
			DOCTYPE, {"provider_message_key": key, "name": ["!=", doc.name]}, "name", for_update=True
		):
			raise
...
def cancel_intent(name: str):
	...
	with control.conversation_fence(conversation):
		...
		if not (
			current.human_owner == user
			or roles & control.MANAGERS
	...
	with control.conversation_fence(conversation):
		...
		if doc.origin == "Automation":
			...
			control.assert_current_generation(
				conversation, doc.conversation_generation, actor_user=doc.actor_user
			)
