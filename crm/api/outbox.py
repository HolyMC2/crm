...
def _projection(doc):
    ...
    result["text"] = (payload.get("text") or {}).get("body", "") if doc.provider == "WhatsApp" else ""
...
def _payload(payload, conversation):
    ...
    if not isinstance(body, str) or not body.strip() or len(body) > 4096:
        frappe.throw("Enter a reply of at most 4096 characters.")
...
def queue_message(conversation, expected_generation, request_id, payload):
    ...
    with control.conversation_fence(conversation):
        ...
        account = frappe.db.get_value("WhatsApp Account", current.account_record,
        ...
        doc = frappe.get_doc({"doctype": DOCTYPE, "name": name, "action_key": name,
            ...
            "source_doctype": "WhatsApp Account", "source_name": account.name, "source_action": account_revision(account),
...
def _result(doc, result):
    ...
    if state == "Accepted":
        ...
        if isinstance(message_id, str) and re.fullmatch(r"wamid\.[^\s]{1,249}", message_id):
