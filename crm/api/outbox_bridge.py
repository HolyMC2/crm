...
ROW_STATUS = {
    "Queued": "Queued", "Claimed": "Queued", "Deferred": "Queued", "Submitting": "Sending",
    "Accepted": "Success", "Blocked": "failed", "Failed": "failed", "Cancelled": "failed", "Unknown": "unknown",
}
...
def reason_message(code, owner=None):
    ...
    if code == "conversation_owned":
        ...
        return _("{0} atiende esta conversación. Solicita el control para responder.").format(name or _("Otra persona"))
    messages = {
        ...
        "customer_window_unverified": _("Pasaron más de 24 horas desde el último mensaje del cliente. Envía una plantilla."),
        ...
        "account_configuration_changed": _("La cuenta de WhatsApp cambió antes del envío. Vuelve a enviarlo."),
        ...
        "provider_response_uncertain": _("No se pudo confirmar el envío. Revisa el chat del cliente antes de reenviarlo."),
        "submission_interrupted": _("No se pudo confirmar el envío. Revisa el chat del cliente antes de reenviarlo."),
        ...
        "conversation_busy": _("Se está enviando otro mensaje de esta conversación. Reintenta en unos segundos."),
...
def _control_refusal(conversation, origin, actor):
    ...
    return {"Bot": "bot_in_control", "Paused": "conversation_paused", "Closed": "conversation_closed"}.get(state, "authority_revoked")
...
def queue_transcript(row, account, payload):
    ...
    with ExitStack() as stack:
        ...
        try:
            frozen = validate_payload(payload, account_id=current.account_id, peer_id=current.peer_id).decode()
        except ValueError:
        ...
        source = frappe.db.get_value("WhatsApp Account", current.account_record,
                                     ["name", "app_id", "business_id"], as_dict=True, for_update=True)
        if not source:
            refuse("account_unavailable")
        kind = payload.get("type")
        purpose = "automation" if origin == "Automation" else "service" if kind == "template" else "manual"
        doc = frappe.get_doc({"doctype": outbox.DOCTYPE, "name": intent_name, "action_key": intent_name,
            "conversation": name, "conversation_generation": current.generation,
            "provider": "WhatsApp", "account_id": current.account_id, "peer_id": current.peer_id,
            "actor_user": actor, "origin": origin, "purpose": purpose, "payload": frozen,
            "source_doctype": "WhatsApp Account", "source_name": source.name, "source_action": account_revision(source),
            "transcript_message": row.name, "state": "Queued", "attempts": 0,
            "expires_at": now_datetime() + timedelta(hours=24),
            "state_log": outbox._canonical([{"state": "Queued", "at": str(now_datetime()), "reason": ""}])})
...
def requeue_transcript(row):
    ...
    """
    name = frappe.db.get_value(outbox.DOCTYPE, {"transcript_message": row.name}, "name") if _bridge_ready() else None
    if not name:
        return False
...
def _publish(row):
    ...
    frappe.publish_realtime("whatsapp_message", {
        "reference_doctype": row.reference_doctype, "reference_name": row.reference_name, "phone": row.to,
    }, after_commit=True)
...
def _insert_projection(doc, name):
    ...
    row = project_accepted(doc, json.loads(doc.payload), name=name, whatsapp_account=conversation.account_record,
                           reference_doctype=reference[0], reference_name=reference[1],
                           sent_by=sent_by, actor=doc.actor_user)
    if row:
        _publish(row)
...
def native_states(rows):
    ...
    if not candidates:
        return rows
    projected = {name[len(PROJECTED_PREFIX):]: name for name in candidates if name.startswith(PROJECTED_PREFIX)}
    ...
    intents = frappe.db.sql(f"""SELECT i.name, i.transcript_message, i.state, i.reason_code, i.attempts,
        ...
        {"rows": tuple(candidates), "intents": tuple(projected) or ("",)}, as_dict=True)
    user = frappe.session.user
    ...
    for intent in intents:
        ...
        may_cancel = (intent.human_owner == user or manager or (intent.origin == "Human" and intent.actor_user == user)
                      or (intent.origin == "Automation" and not intent.human_owner))
        row["native"] = {"intent": intent.name, "state": intent.state,
            "reason": reason_message(intent.reason_code) if intent.reason_code else "",
            "can_retry": bool(may_retry and unsent and intent.state in {"Blocked", "Failed", "Deferred"}
                              and intent.attempts < outbox.MAX_ATTEMPTS),
            "can_cancel": bool(may_cancel and unsent and intent.state in {"Queued", "Claimed", "Blocked", "Deferred", "Failed"})}
    return rows


def _bridge_ready():
    return frappe.db.exists("DocType", outbox.DOCTYPE) and frappe.db.has_column(outbox.DOCTYPE, "transcript_message")
...
def thread_control(reference_doctype, reference_name, phone, whatsapp_account=None):
    ...
    if reference_doctype not in ("CRM Deal", "CRM Lead") or not _record_number(reference_doctype, reference_name, peer):
    ...
    return {key: detail.get(key) for key in ("name", "control_state", "human_owner", "generation", "provider_control",
            "allowed_actions", "manager_reason_required", "actor", "control_requests")} | {
...
def _send_account(whatsapp_account=None):
    ...
    if whatsapp_account:
        return frappe.db.get_value("WhatsApp Account", whatsapp_account, ["name", "phone_id", "mode", "status"], as_dict=True)
    from frappe_whatsapp.utils import get_whatsapp_account
    default = get_whatsapp_account(account_type="outgoing")
    return default and frappe.db.get_value("WhatsApp Account", default.name, ["name", "phone_id", "mode", "status"], as_dict=True)
...
def assert_send_account(whatsapp_account):
    ...
    probe = frappe._dict(provider="WhatsApp", account_id=account.phone_id, shop_key=None,
