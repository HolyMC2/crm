...
def _seconds(value):
	if type(value) is int:
		value = str(value)
	_require(
		isinstance(value, str) and re.fullmatch(r"[0-9]{1,12}", value), "customer_activity_timestamp_invalid"
	)
	result = int(value)
...
def _receipt(receipt_name, provider, account_id, peer_id, provider_timestamp):
	_require(
		...
		"customer_activity_worker_required",
	)
	...
	_require(
		...
		and row.provider == provider == "WhatsApp"
	...
	try:
		...
		_require(
			not any(
				key in value
				for key in ("statuses", "message_echoes", "history", "state_sync", "smb_app_state_sync")
			)
		)
...
def internal_apply_customer_activity(provider, account_id, peer_id, *, receipt_name, provider_timestamp):
	...
	with control.conversation_fence(name):
		...
		accounts = frappe.db.get_values(
			"WhatsApp Account",
			{"phone_id": account_id},
			["name", "phone_id", "app_id", "business_id", "status", "mode"],
			as_dict=True,
			for_update=True,
		)
		_require(
			...
			and accounts[0].business_id == payload["business_id"],
		...
		_require(
			...
			and doc.account_id == account_id
			and doc.peer_id == peer_id
			and doc.account_record == current_account.name
			and (doc.shop_key or "") == (current_account.shop or ""),
			"customer_activity_account_unavailable",
		)
		key = control._event_key(name, "Provider", receipt_name, "customer_reply")
		fingerprint = control._digest(
			[provider, account_id, peer_id, "customer_reply", timestamp, row.payload_hash]
		)
