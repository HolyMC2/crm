			...
			result["sections"].append(
				{
					...
					"render_in": section["render_in"],
					"enabled": 1,
					"idx": frappe.utils.cint(section.get("idx")),
					"config": {"initiallyOpen": options.get("initiallyOpen") is True}
					if component == "RepairOrdersSection" and isinstance(options, dict)
					else {},
				}
			)
	except Exception:
