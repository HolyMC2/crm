...
ACCOUNT_TYPES = {"Webchat": "CRM Webchat Channel", "WhatsApp": "WhatsApp Account",
                 "Messenger": "Messenger Page", "Instagram": "Messenger Page"}
MAX_COOLDOWN = 7 * 24 * 60
_SERVICE_TOKEN = object()
PUBLIC = ("name", "title", "provider", "account_id", "account_record", "flow", "flow_hash", "department",
          "cooldown_minutes", "status", "revision", "published_revision", "bot_binding", "bot_profile",
          "published_by", "published_at", "paused_by", "paused_at", "pause_reason", "modified")
...
def policy_hash(doc):
    return control._digest([1, doc.name, doc.provider, doc.account_id, doc.account_record or None,
        doc.bot_binding or None, doc.bot_profile or None, doc.flow, doc.flow_hash or None,
        doc.department or None, cint(doc.cooldown_minutes), cint(doc.published_revision)])
...
def _probe(provider, account_id, user=None, write=True):
    control.conversation_key(provider, account_id, "0" * 64 if provider == "Webchat" else "1")
    probe = frappe._dict(provider=provider, account_id=account_id, name=None, shop_key=None,
...
def save_policy(values, name=None, expected_revision=None):
    ...
    if name:
        ...
        doc = frappe.get_doc({"doctype": DOCTYPE, "status": "Draft", "revision": 1,
                              "cooldown_minutes": 1440, **{k: values.get(k) for k in EDITABLE if k in values}})
...
def _site_ready():
    reasons = []
    for doctype, field in (("FCRM Settings", "conversation_automation_enabled"), ("Marketing Settings", "enable_automation")):
        value = frappe.db.sql("SELECT value FROM `tabSingles` WHERE doctype=%s AND field=%s", (doctype, field))
...
def resolve_route(provider, account_record):
    ...
    binding = bot_policy.resolve_route(provider=provider, account_type=ACCOUNT_TYPES[provider],
                                       account=account_record, entry_point="default")
    if not binding:
        return None, None, [_("Create and activate a customer bot connection for this exact account.")]
    connection = frappe.db.get_value("Bot Channel Binding", binding,
        ["name", "bot_profile", "active", "activated_revision"], as_dict=True)
    profile = frappe.db.get_value("Bot Profile", connection.bot_profile,
        ["name", "audience", "paused", "published_revision"], as_dict=True)
    reasons = []
    if not profile or profile.audience != "Customer":
        reasons.append(_("The account's bot must be a customer bot."))
    elif not connection.active or profile.paused or connection.activated_revision != profile.published_revision:
    ...
    if not adapter:
        ...
        if profile and flow.get("run_as_user"):
            ...
            if published.get("execution_user") != flow.get("run_as_user"):
                reasons.append(_("The flow and the account's bot must use the same dedicated execution User."))
    other = frappe.db.get_value(DOCTYPE, {"route_key": route_key(doc.provider, doc.account_id), "name": ["!=", doc.name]}, "name")
    if other:
        reasons.append(_("Another published policy already owns this account. Pause it first."))
    state = {
        "account_record": account_record, "bot_binding": connection.name if connection else None,
        "bot_profile": profile.name if profile else None,
        "bot_revision": profile.published_revision if profile else None,
        "flow_hash": flow.get("hash"), "flow_department": flow.get("department"),
        "blockers": reasons,
    }
    state["review_hash"] = control._digest([doc.name, cint(doc.revision), state["account_record"], state["bot_binding"],
                                           state["bot_profile"], state["bot_revision"], state["flow_hash"], reasons])
    return state
...
def publish_policy(name, expected_revision, review_hash, approval_note):
    ...
    doc.update({"status": "Published", "account_record": state["account_record"], "bot_binding": state["bot_binding"],
                "bot_profile": state["bot_profile"], "flow_hash": state["flow_hash"],
                "department": doc.department or state.get("flow_department"),
                "published_revision": cint(doc.revision), "published_by": frappe.session.user,
                "published_at": now_datetime(), "approval_note": note,
                "paused_by": None, "paused_at": None, "pause_reason": None,
...
def pause_policy(name, reason=None):
    ...
    if doc.status != "Published":
        return projection(doc)
    doc.update({"status": "Paused", "route_key": None, "published_hash": None, "paused_by": frappe.session.user,
                "paused_at": now_datetime(), "pause_reason": control._text(reason, 500, required=False) or None})
    _mark(doc).save(ignore_permissions=True)
    return projection(doc)
...
def published_for(provider, account_id):
    ...
    if not frappe.db.exists("DocType", DOCTYPE):
        return None
    name = frappe.db.get_value(DOCTYPE, {"route_key": route_key(provider, account_id), "status": "Published"},
...
def grant_reason(policy, conversation):
    ...
    if policy.route_key != route_key(policy.provider, policy.account_id) or policy.published_hash != policy_hash(policy):
        return "automation_policy_changed"
    if (policy.provider, policy.account_id) != (conversation.provider, conversation.account_id) \
    ...
    if reasons or not connection or connection.name != policy.bot_binding or profile.name != policy.bot_profile:
...
def cooldown_reason(policy, conversation):
    ...
    recent = frappe.db.sql(f"""SELECT name FROM `tab{control.EVENT}` WHERE conversation=%s AND creation>=%s
        ...
        LIMIT 1""", (conversation.name, since))
