...
def _live_claim(row):
	try:
		return (
			...
			and get_datetime(row.lease_until) > now_datetime()
		)
...
def _timestamp(value):
	if type(value) is int:
		value = str(value)
	_require(
		isinstance(value, str) and re.fullmatch(r"[0-9]{1,12}", value), "native_delivery_timestamp_invalid"
	)
	seconds = int(value)
...
def _receipt(receipt_name, expected_entry):
	_require(
		not getattr(frappe, "request", None)
		and receipt_name
		and receipt_name == frappe.flags.get("meta_webhook_receipt"),
		"native_delivery_worker_required",
	)
	...
	_require(
		control._digest([row.provider, row.app_id, row.account_id, row.event_type, row.event_id])
		== row.event_key
		== row.name
	)
	try:
		...
		_require(
			...
			and re.fullmatch(r"wamid\.[^\s]{1,249}", entry["id"])
			and not entry["id"].startswith("wamid.demo-")
		)
		_require(entry["status"] in {"sent", "delivered", "read", "failed"})
		_require(
			isinstance(entry["recipient_id"], str) and re.fullmatch(r"[0-9]{1,40}", entry["recipient_id"])
		)
...
def apply_delivery_receipt(receipt_name, *, expected_entry=None, account_records=None):
	...
	with control.conversation_fence(name):
		...
		_require(
			len(accounts) == 1
			and accounts[0].status == "Active"
			and accounts[0].mode == "Live"
			and accounts[0].app_id == row.app_id
			and accounts[0].business_id == business,
			"native_delivery_account_unavailable",
		)
		account = accounts[0]
		_require(
			account_records is None
			or (isinstance(account_records, tuple | list) and tuple(account_records) == (account.name,)),
			"native_delivery_account_unavailable",
		)
		...
		_require(
			current.account_record == scoped_account.name
			and (current.shop_key or "") == (scoped_account.shop or ""),
			"native_delivery_account_changed",
		)
		try:
		...
		_require(
			doc.state in {"Accepted", "Delivered", "Read", "Failed"}, "native_delivery_state_unavailable"
		)
		wanted = {"sent": "Accepted", "delivered": "Delivered", "read": "Read", "failed": "Failed"}[
			entry["status"]
		]
		rank = {"Accepted": 1, "Delivered": 2, "Read": 3}
		unchanged = (
			doc.state == wanted
			or (doc.state in {"Delivered", "Read"} and rank.get(wanted, 0) < rank[doc.state])
			or (doc.state == "Failed" and wanted == "Accepted")
		)
		...
		if not unchanged:
			values = (
				...
				if wanted == "Delivered"
				else {"read_at": observed_at}
				if wanted == "Read"
				else {}
			)
			outbox._transition(
				doc, wanted, "provider_delivery_failed" if wanted == "Failed" else "", **values
			)
