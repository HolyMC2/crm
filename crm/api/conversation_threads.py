...
def _actor():
	...
	if not roles.intersection(control._channel_roles("WhatsApp")) and not roles.intersection(
		control.departments.member_roles()
	):
		control._deny()
	return actor
...
def _channel_available(provider):
	...
	if provider == "Webchat":
		return all(
			...
			for dt in ("CRM Webchat Channel", "CRM Webchat Message", "CRM Webchat Session")
		)
	if provider == "WhatsApp":
		return (
			"frappe_whatsapp" in apps
			and frappe.db.exists("DocType", "WhatsApp Account")
			and frappe.db.exists("DocType", "WhatsApp Message")
		)
	return (
		"doco_marketing" in apps
		and frappe.db.exists("DocType", "Messenger Page")
		and frappe.db.exists("DocType", "Messenger Message")
	)
...
def _next(position, context):
	...
	return encrypt(
		json.dumps({"context": [frappe.session.user, *context], "position": position}, default=str)
	)
...
def _scope(provider, account_id, doc=None):
	...
	if provider == "Webchat":
		return frappe._dict(
			doctype="CRM Webchat Message",
			table="`tabCRM Webchat Message`",
			peer="m.peer_id",
			peer_match="m.peer_id=%s",
			where="m.channel=%s AND m.direction IN ('Incoming','Outgoing')",
			args=[account.name],
	...
	if provider == "WhatsApp":
		...
		return frappe._dict(
			...
			where="m.whatsapp_account=%s AND m.type IN ('Incoming','Outgoing')",
			args=[account.name],
			timestamp="COALESCE(m.external_sent_at,m.creation)"
			if frappe.db.has_column("WhatsApp Message", "external_sent_at")
			else "m.creation",
		)
	page_id = frappe.db.get_value("Messenger Page", account.name, "page_id", for_update=control._locked())
	platform = (
		"(m.platform='Messenger' OR m.platform IS NULL OR m.platform='')"
		if provider == "Messenger"
		else "m.platform='Instagram'"
	)
	return frappe._dict(
		doctype="Messenger Message",
		table="`tabMessenger Message`",
		peer="m.psid",
		peer_match="m.psid=%s",
		where=f"m.page_id=%s AND {platform} AND m.direction IN ('Incoming','Outgoing')"
		" AND COALESCE(NULLIF(m.event_type,''),'message') IN ('message','echo')",
		args=[page_id],
...
def _metadata(scope, peer, *, cursor=None, limit=SCAN):
	...
	references = (
		"NULL AS reference_doctype, NULL AS reference_name"
	...
	return frappe.db.sql(  # nosemgrep: frappe-sql-format-injection
        ...
        ORDER BY {scope.timestamp} DESC,m.name DESC LIMIT {int(limit)}{control._for_update()}""",
		args,
		as_dict=True,
	)
...
def _first_visible(scope, peer):
	...
	rows = frappe.db.sql(  # nosemgrep: frappe-sql-format-injection
        ...
        ORDER BY {scope.timestamp} DESC,m.name DESC LIMIT 1{control._for_update()}""",
		[*scope.args, peer, *args],
		as_dict=True,
	)
	return rows[0] if rows else None
...
def _safe_attachment(value, doctype, message_name):
	...
	if f.attached_to_doctype == doctype and f.attached_to_name == message_name:
		pass
	elif f.attached_to_doctype in REFS and _source_allowed(
		{"reference_doctype": f.attached_to_doctype, "reference_name": f.attached_to_name}
	):
		pass
	else:
		return None
...
def _message(scope, metadata):
	...
	if scope.doctype == "CRM Webchat Message":
		row = frappe.db.get_value(
			scope.doctype, metadata.name, ["direction", "text"], as_dict=True, for_update=locked
		)
	...
	if content_type != "text" and not text:
		text = {"image": "Imagen", "video": "Video", "audio": "Audio", "document": "Documento"}.get(
			content_type, "Mensaje sin texto"
		)
	return {
		"id": metadata.name,
		...
		"attach": attachment,
		"attachment_restricted": bool(row.attach and not attachment),
...
def _detail(doc):
	...
	result["send_available"] = bool(
		doc.control_state == "Human"
		and doc.human_owner == frappe.session.user
		and "release" in result["allowed_actions"]
		and doc.provider_control in ("Not Applicable", "Ours")
		and frappe.db.exists("DocType", "CRM Outbound Intent")
	...
	result["control_requests"] = frappe.db.get_values(
		control.EVENT,
...
def _reference_item(provider, account_id, peer, doc=None):
	...
	return {
		...
		"materialized": bool(doc),
		"control_state": doc.control_state if doc else "Human",
...
def _implied_threads(doctype, name, seen):
	...
	if _channel_available("WhatsApp"):
		rows = frappe.db.sql(
			"""SELECT m.whatsapp_account AS account,
	...
	if _channel_available("Messenger"):
		rows = frappe.db.sql(
			"""SELECT m.page_id, m.psid, COALESCE(m.platform,'') AS platform
...
def list_accounts():
	...
	for provider in ("WhatsApp", "Messenger", "Instagram", "Webchat"):
		...
		if provider == "Webchat":
			...
			doctype, field, label, status = (
				"Messenger Page",
				"page_id" if provider == "Messenger" else "ig_account_id",
				"page_name",
				"enabled",
			)
		for row in frappe.db.get_values(
			doctype, {}, ["name", field, label, status], as_dict=True, for_update=control._locked()
		):
...
def list_threads(provider: str, account_id: str, cursor: str | None = None, limit: int | str = 30):
	...
	peers = frappe.db.sql(  # nosemgrep: frappe-sql-format-injection
        ...
        ORDER BY peer_id LIMIT {SCAN + 1}""",
		[provider, account_id, *scope.args, "^" + pattern + "$", after],
		as_dict=True,
	)
	items, position = [], None
...
def _collapse_spellings(provider, account_id, rows):
	...
	keys = {
		control.conversation_key(provider, account_id, p): p
	...
	if keys:
		known = {
			keys[n]
			for n in frappe.get_all(control.DOCTYPE, filters={"name": ["in", list(keys)]}, pluck="name")
		}
	out = []
	for members in groups.values():
...
def list_operators(conversation: str, query: str | None = ""):
	...
	candidates = frappe.db.sql(
        ...
        AND (name LIKE %s OR full_name LIKE %s) ORDER BY full_name,name LIMIT 100""",
		("%" + query + "%", "%" + query + "%"),
		as_dict=True,
	)
	return [
		{"name": row.name, "label": row.full_name or row.name}
		for row in candidates
		if _permitted(lambda: control._authorize(doc, row.name, write=True))
	][:30]
