...
def _reference(doc, user):
    ...
    if not frappe.has_permission(doc.reference_doctype, "read", doc=record, user=user, print_logs=False):
        return None
    return {"doctype": doc.reference_doctype, "name": doc.reference_name,
...
def _item(doc, user, names=None):
    ...
    display = (names or {}).get(doc.peer_id) or threads._display_names(doc.provider, [doc.peer_id]).get(doc.peer_id, doc.peer_id)
    return {
        "name": doc.name, "provider": doc.provider, "account_id": doc.account_id, "display_name": display,
        "department": doc.get("department") or None, "control_state": doc.control_state,
        "human_owner": doc.human_owner, "generation": doc.generation, "state": state, "next_action": next_action,
        "automation_state": doc.get("automation_state") or None, "automation_run": doc.get("automation_run") or None,
        "reference": _reference(doc, user), "record_links": control.context_view(doc, user),
        "routed_at": doc.get("routed_at"), "modified": doc.modified,
...
def _scan(wanted, view, user, after=None, limit=PAGE):
    ...
    rows = frappe.db.sql(f"""SELECT name, modified FROM `tabCRM Conversation`
        WHERE department IN %(departments)s{clause}{boundary}
        ORDER BY modified DESC, name DESC LIMIT {SCAN + 1}""", values, as_dict=True)
...
def list_departments():
    roles = _roles()
    return {"schema_version": SCHEMA_VERSION, "departments": departments.describe(roles),
            "can_manage": _can_manage(roles)}


@frappe.whitelist()
def list_queue(department=None, view="all", cursor=None, limit=30):
    ...
    if after is not None and (not isinstance(after, list) or len(after) != 2 or not all(isinstance(v, str) for v in after)):
    ...
    for provider in {doc.provider for doc in docs}:
        names.update(threads._display_names(provider, [doc.peer_id for doc in docs if doc.provider == provider]))
    return {"items": [_item(doc, user, names) for doc in docs], "departments": wanted,
...
def queue_counts(roles=None, user=None):
    ...
    for department in departments.member_departments(roles):
        ...
        counts = {"total": len(docs), "unassigned": 0, "mine": 0, "bot": 0, "waiting_input": 0, "blocked": 0,
                  "capped": bool(more)}
        for doc in docs:
            state, _next = _queue_state(doc, user)
            key = {"Unassigned": "unassigned", "Paused": "unassigned", "Mine": "mine", "Bot": "bot",
                   "Waiting input": "waiting_input", "Blocked": "blocked"}.get(state)
            if key:
                counts[key] += 1
...
def route_conversation(name, department, expected_generation, command_id, reason=None):
    return control.route(name, department or None, expected_generation, command_id, reason)


@frappe.whitelist(methods=["POST"])
def link_record(name, doctype, docname, command_id):
    control.link_record(name, doctype, docname, command_id)
    return get_context(name)


@frappe.whitelist(methods=["POST"])
def unlink_record(name, doctype, docname, command_id):
...
def _connection(account, policy_rows):
    ...
    published = next((row for row in policy_rows if row["provider"] == provider and row["account_id"] == account_id
                      and row["status"] == "Published"), None)
    return {"name": binding or f"{provider}:{account_id}", "bot_profile": profile, "provider": provider,
            "account": account.get("label") or account_id, "account_id": account_id,
            "active": bool(account.get("active") and binding and not blockers),
            "send_ready": bool(channel_send_ready(provider)), "automation_ready": ready,
            "policy": published, "blockers": blockers}
