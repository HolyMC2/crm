...
def _origin(value):
	...
	try:
		...
		_require(
			len(host) <= 253 and re.fullmatch(r"[a-z0-9](?:[a-z0-9.-]*[a-z0-9])?", host) and ".." not in host
		)
		port = parsed.port
...
def _guest(fn=None, *, scrub_after=True):
	...
	def protected(*args, **kwargs):
		try:
			...
			if isinstance(error, frappe.PermissionError):
				status, reason, message = (
					403,
					"session_unavailable",
					"Webchat session or channel is unavailable.",
				)
		...
		response = Response(
			_canonical({"message": message, "reason": reason}),
			status=status,
...
def _request_authority():
	...
	request = getattr(frappe.local, "request", None)
	if (
		type(context) is not _RequestAuthority
		or context.request is not request
		or context.path != request.path
	):
		_deny()
	return context
...
def validate_channel(doc):
	...
	_short(doc.profile)
	_require(
		doc.public_origin == _origin(doc.public_origin)
		and doc.binding_key == _key(doc.profile, doc.public_origin)
	)
	...
	if not doc.is_new():
		old = frappe.db.get_value(
			CHANNEL,
			doc.name,
			["account_id", "profile", "public_origin", "binding_key"],
			as_dict=True,
			for_update=True,
		)
		_require(
			old and all(doc.get(field) == old.get(field) for field in old),
			"Webchat channel binding is immutable.",
		)
...
def validate_session(doc):
	...
	if not doc.is_new():
		old = frappe.db.get_value(
			SESSION,
			doc.name,
			["channel", "peer_id", "capability_hash", "expires_at", "creation", "revoked"],
			as_dict=True,
			for_update=True,
		)
		_require(
			old
			and all(str(doc.get(field)) == str(old.get(field)) for field in old if field != "revoked")
			and old.revoked == 0
			and doc.revoked == 1,
			"Webchat sessions may only be revoked.",
		)
...
def validate_message(doc):
	...
	_require(
		doc.text_hash == _hash(_text(doc.text))
		and type(doc.control_generation) is int
		and doc.control_generation >= 1
	)
...
def _channel(channel_id, active=True):
	...
	row = frappe.db.get_value(
		CHANNEL,
		channel_id,
		["name", "account_id", "enabled", "label", "profile", "public_origin", "binding_key", "modified"],
		as_dict=True,
		for_update=True,
	)
...
def _config_projection(row):
	return {
		key: str(row.get(key)) if key == "modified" else row.get(key)
...
def list_channels():
	...
	_manager()
	rows = frappe.db.sql(
		"""SELECT account_id,label,profile,public_origin,enabled,modified
        FROM `tabCRM Webchat Channel` ORDER BY label,name LIMIT 101""",
		as_dict=True,
	)
...
def configure_channel(
	...
	_require(
		(type(enabled) in {bool, int} and enabled in (0, 1))
		or (type(enabled) is str and enabled in {"0", "1"})
	)
	creating = channel_id is None
	if creating:
		_require(expected_modified is None)
		_require(
			not frappe.db.get_value(CHANNEL, {"binding_key": _key(profile, public_origin)}, "name"),
		...
		doc = frappe.get_doc(
			{
				...
				"binding_key": _key(profile, public_origin),
				"profile": profile,
		...
		_require(
			isinstance(expected_modified, str) and str(doc.modified) == expected_modified,
			"Webchat channel changed. Reload before saving.",
		)
		_require(
			doc.profile == profile and doc.public_origin == public_origin,
			"Webchat channel binding is immutable.",
		)
...
def current_session(channel_id, peer_id, *, allow_revoked=False):
	...
	if (
		...
		or doc.peer_id != peer_id
		or (doc.revoked and not allow_revoked)
...
def _visitor(channel_id, *, allow_revoked=False):
	...
	hint = frappe.db.get_value(
		SESSION, {"channel": channel_id, "capability_hash": digest}, ["name", "peer_id"], as_dict=True
	)
	if not hint:
		_deny()
...
def _message_projection(row):
	return {
		...
		"created_at": _public_time(row.creation),
	}
...
def _history(session, conversation, cursor=None):
	...
	if cursor is not None:
		...
		anchor = frappe.db.get_value(
			MESSAGE, cursor, ["channel", "session", "peer_id", "creation"], as_dict=True, for_update=True
		)
		_require(
	...
	return {
		"messages": [_message_projection(row) for row in reversed(selected)],
...
def _existing(key, session, direction, request_id, text):
	row = frappe.db.get_value(
		MESSAGE,
		key,
		["name", "channel", "session", "peer_id", "direction", "request_id", "text", "text_hash", "creation"],
		as_dict=True,
		for_update=True,
	)
	if row:
		_require(
			(row.channel, row.session, row.peer_id, row.direction, row.request_id, row.text, row.text_hash)
...
def send(channel_id: JSONValue, request_id: JSONValue, text: JSONValue):
	...
	with _visitor(channel_id) as (session, conversation):
		...
		if existing:
			return {
				**_history(session, conversation),
				"message": _message_projection(existing),
				"replayed": True,
			}
		from crm.api import conversations as control
...
def deliver_local(intent, payload):
	...
	if not _existing(key, session, "Outgoing", intent.name, frozen["text"]):
		_insert(
			session, conversation, "Outgoing", intent.name, frozen["text"], intent.conversation_generation
		)
