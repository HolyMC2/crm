...
class TestConversationActivitySql(unittest.TestCase):
	def setUp(self):
		...
		self.account = frappe.get_doc(
			{
				...
				"account_name": "activity-" + uuid4().hex,
		...
		for target in (
			...
			self.enterContext(
				patch(target, side_effect=AssertionError("customer activity invoked unrelated side effect"))
			)
		self.enterContext(patch.object(frappe, "request", None))
		self.enterContext(
			patch.object(control, "get_or_create", side_effect=AssertionError("activity created identity"))
		)
	...
	def seed(self, state, *, boundary=None, generation=7, owner=None):
		frappe.db.set_value(
			...
			{
				"control_state": state,
				"bot_enabled": int(state == "Bot"),
				"human_owner": owner,
				"generation": generation,
				"modified": boundary or system_time(self.now - 60),
			},
	...
	def receipt(self, *, timestamp=None, peer=None, event_type="message", kind="text", account=None):
		...
		frappe.db.set_value(
			...
			name,
			{"state": "Processing", "attempts": 1, "lease_until": now_datetime() + timedelta(minutes=5)},
			update_modified=False,
		)
	...
	def apply(self, receipt, timestamp=None, **kwargs):
		return activity.internal_apply_customer_activity(
			"WhatsApp",
			self.account_id,
			self.peer,
			receipt_name=receipt.name,
			provider_timestamp=self.now if timestamp is None else timestamp,
			**kwargs,
		)
	...
	def test_guest_fresh_reply_holds_bot_and_invalidates_old_followup_generation(self):
		...
		self.assertEqual(
			(result["control_state"], result["generation"], result["bot_enabled"]), ("Human", 8, 0)
		)
		...
		with control.conversation_fence(self.doc.name), self.assertRaises(frappe.TimestampMismatchError):
			control.assert_current_generation(
				self.doc.name, 7, origin="Bot", actor_user="Guest", run_name="pending-followup"
			)
		event = frappe.db.get_value(
			control.EVENT,
			{"source_receipt": receipt.name},
			["action", "origin", "actor_user", "result_json"],
			as_dict=True,
		)
	...
	def test_old_same_second_and_microsecond_boundary_do_not_revoke_control(self):
		for boundary in (
			system_time(self.now + 1),
			system_time(self.now),
			system_time(self.now) + timedelta(microseconds=1),
		):
			...
			self.doc.reload()
			self.assertEqual(
				(self.doc.control_state, self.doc.generation, self.doc.bot_enabled), ("Bot", 7, 1)
			)
	...
	def test_future_message_and_its_later_replay_never_become_a_new_command(self):
		...
		with self.context(receipt):
			...
			frappe.db.set_value(
				"Meta Webhook Receipt",
				receipt.name,
				{"attempts": 2, "lease_until": system_time(timestamp + 300)},
				update_modified=False,
			)
	...
	def test_receipt_arrival_time_is_not_activity_time(self):
		...
		with self.context(receipt):
			self.assertEqual(
				self.apply(receipt, self.now - 120)["reason_code"], "customer_activity_precedes_control"
			)
	...
	def test_human_owner_paused_and_closed_are_preserved(self):
		for state in ("Human", "Paused", "Closed"):
			...
			with self.context(receipt):
				result = self.apply(receipt)
			self.assertEqual(
				(result["control_state"], result["human_owner"], result["generation"], result["bot_enabled"]),
	...
	def test_fresh_or_stale_reply_never_materializes_missing_conversation(self):
		...
		for timestamp in (self.now, self.now - 86400):
			...
			with self.context(receipt):
				result = activity.internal_apply_customer_activity(
					"WhatsApp",
					self.account_id,
					other,
					receipt_name=receipt.name,
			...
			self.assertFalse(
				frappe.db.exists(
					control.DOCTYPE, control.conversation_key("WhatsApp", self.account_id, other)
				)
			)
		...
		private = frappe.get_doc(
			{
				...
				"name": "activity-private-" + uuid4().hex,
				"channel": "WhatsApp",
	...
	def test_forged_context_claimed_timestamp_and_peer_are_denied(self):
		...
		with self.context(receipt):
			...
			with self.assertRaises(activity.CustomerActivityError):
				activity.internal_apply_customer_activity(
					"WhatsApp",
					self.account_id,
					self.peer + "1",
					receipt_name=receipt.name,
	...
	def test_processing_flag_without_live_claim_cannot_mutate_control(self):
		for values in (
			...
			with (
				...
				self.assertRaisesRegex(activity.CustomerActivityError, "customer_activity_claim_required"),
			):
				self.apply(receipt)
		self.doc.reload()
		self.assertEqual((self.doc.control_state, self.doc.generation), ("Bot", 7))
		...
		for field, value in (
			...
			with (
				...
				self.assertRaisesRegex(
					activity.CustomerActivityError, "customer_activity_account_unavailable"
				),
			):
				self.apply(receipt)
	...
	def test_account_record_reassignment_cannot_hold_wrong_conversation(self):
		frappe.db.set_value(
			control.DOCTYPE, self.doc.name, "account_record", "other-account", update_modified=False
		)
		receipt = self.receipt()
		with (
			self.context(receipt),
			self.assertRaisesRegex(activity.CustomerActivityError, "customer_activity_account_unavailable"),
		):
			self.apply(receipt)
	...
	def test_hold_and_audit_share_outer_rollback_and_prior_request_work_survives(self):
		...
		with (
			self.context(receipt),
			patch.object(control, "_persist_transition", side_effect=fail_after_projection),
		):
