...
class TestWebchat(unittest.TestCase):
	def setUp(self):
		...
		self.enterContext(
			patch("requests.sessions.Session.request", side_effect=AssertionError("Transport forbidden"))
		)
	...
	def test_bootstrap_private_random_identity_and_no_conversation(self):
		...
		self.assertEqual(
			self.rpc("history", capability=self.capability, channel_id=session.channel),
	...
	def test_channel_and_session_isolation(self):
		first = self.send()
		self.failure(
			403, lambda: self.rpc("history", capability=self.capability, channel_id=self.other["account_id"])
		)
	...
	def test_message_and_control_failure_rollback_together(self):
		...
		with patch.object(
			control, "internal_webchat_customer_reply", side_effect=RuntimeError("fictional database failure")
		):
	...
	def test_manager_binding_immutable_current_revision_default_off(self):
		...
		self.assertFalse(
			self.rpc("get_channel", profile=new["profile"], public_origin=new["public_origin"])["available"]
		)
		...
		args = {
			"label": new["label"],
			"profile": new["profile"],
			"public_origin": new["public_origin"],
			"enabled": "1",
			"channel_id": new["account_id"],
			"expected_modified": new["modified"],
		}
		...
		with self.assertRaises(frappe.ValidationError):
			api.configure_channel(
				**{
					**args,
					"expected_modified": result["modified"],
					"public_origin": "https://different.example.invalid",
				}
			)
		with (
			patch.object(frappe.local, "session", frappe._dict(user="Guest")),
	...
	def test_manager_saved_channel_listing_safe_bounded_and_current(self):
		...
		changed = api.configure_channel(
			label="Updated fictional label",
			profile=own["profile"],
			public_origin=own["public_origin"],
			enabled=0,
			channel_id=own["account_id"],
			expected_modified=own["modified"],
		)
	...
	def test_core_private_document_reads_denied_and_native_mutations_still_work(self):
		...
		with patch.object(frappe.local, "conf", frappe._dict(frappe.conf)):
			...
			intent = outbox.queue_message(
				name, 2, uuid4().hex, {"type": "text", "text": "Private fictional reply"}
			)
	...
	def test_message_refresh_is_after_commit_exact_authorized_user_id_only(self):
		...
		self.realtime.assert_called_once_with(
			"crm_conversation_updated", {"name": name}, user="Administrator", after_commit=True
		)
		self.realtime.reset_mock()
		self.send(request_id="owned")
		self.realtime.assert_not_called()
	...
	def test_raw_boundary_rejects_extra_duplicate_query_size_and_nonfinite(self):
		...
		cases = [
			dict(raw=json.dumps({**args, "peer_id": "a" * 64}).encode()),
			dict(raw=b'{"capability":"one","capability":"two"}'),
			dict(query_string="capability=forbidden"),
			dict(raw=b" " * (api.MAX_BODY_BYTES + 1)),
			dict(raw=b'{"capability":NaN}'),
			dict(method="GET"),
			dict(mimetype="text/plain"),
		]
		for options in cases:
			with (
				self.subTest(options=list(options)),
				self.request(args, capability=self.capability, **options),
			):
	...
	def test_unexpected_failure_scrubs_diagnostics_and_never_exposes_exception(self):
		args = {"channel_id": self.channel["account_id"]}
		with (
			self.request(args, capability=self.capability) as request,
			patch.object(api, "_public_rate", side_effect=RuntimeError("private diagnostic")),
		):
			api.prepare_request()
	...
	def test_actual_recorder_captured_metadata_is_excluded_without_global_change(self):
		...
		with self.request(args, capability=self.capability) as request:
			...
			with (
				...
				self.assertNotIn(
					self.capability, json.dumps({"headers": recorder.headers, "form": recorder.form_dict})
				)
	...
	def test_recorder_exclusion_failure_refuses_request_with_static_error(self):
		...
		with (
			self.request({"channel_id": self.channel["account_id"]}, capability=self.capability),
			patch.object(frappe.local, "_recorder", recorder, create=True),
		):
	...
	def test_redis_quota_and_outage_are_fail_closed_hashed(self):
		...
		for result in (31, None, "1"):
			with (
				patch.object(frappe.cache, "eval", return_value=result),
				self.assertRaises(frappe.RateLimitExceededError),
			):
				REAL_RATE("visitor", self.capability, 30)
		with (
			patch.object(frappe.cache, "eval", side_effect=RuntimeError("Redis unavailable")),
			self.assertRaises(frappe.RateLimitExceededError),
		):
			REAL_RATE("visitor", self.capability, 30)
	...
	def test_shared_egress_budget_does_not_replace_tighter_channel_limit(self):
		...
		with self.request({"channel_id": channel}, endpoint="bootstrap"):
			...
			self.assertEqual(
				self.rate.call_args_list,
				[
					call("ip", "192.0.2.88", 6000),
					call("channel", channel, 600),
					call("bootstrap_ip", "192.0.2.88", 3000, 3600),
	...
	def test_payload_and_local_dispatch_guard_first(self):
		...
		self.assertEqual(
			api.validate_payload(
				{"type": "text", "text": "literal <b>"}, account_id=session.channel, peer_id=session.peer_id
			),
			expected,
		)
		with self.assertRaises(ValueError):
			api.validate_payload(
				{"type": "text", "text": "x", "reference_name": "secret"},
				account_id=session.channel,
		...
		with (
			patch.object(outbox, "require_dispatch", side_effect=frappe.PermissionError),
	...
	def test_local_delivery_is_stable_one_row_and_has_no_transaction_boundary(self):
		...
		with (
			patch.object(outbox, "require_dispatch") as guard,
	...
	def test_queue_contention_has_distinct_static_pending_without_transaction_reset(self):
		...
		name = self.conversation()
		with (
			patch.object(control, "_load", side_effect=frappe.QueryDeadlockError("private SQL diagnostic")),
			patch.object(frappe.db, "commit") as commit,
			patch.object(frappe.db, "rollback") as rollback,
		):
			with self.assertRaises(outbox.ReplyRequestPending) as pending:
				outbox.queue_message(
					name, 1, "unchanged-id", {"type": "text", "text": "Fictional frozen reply"}
				)
			self.assertEqual(
				str(pending.exception), "Request could not be confirmed. Check the same request again."
			)
