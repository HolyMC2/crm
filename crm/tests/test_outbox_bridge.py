...
class TestOutboxBridge(test_outbox.TestOutbox):
    @contextmanager
    def person(self, user=None, cmd="crm.api.whatsapp.create_whatsapp_message", path=None, form=None, reply=True):
    ...
    def row(self, text="Fictional reply", **fields):
        doc = frappe.get_doc({"doctype": "WhatsApp Message", "type": "Outgoing", "to": self.peer,
            "whatsapp_account": self.account.name, "message": text, "content_type": "text",
            "doco_sent_by_type": "Human", **fields})
    ...
    def status(self, row):
        return frappe.db.get_value("WhatsApp Message", row.name, ["status", "message_id", "failure_reason"], as_dict=True)
    ...
    def test_outbox_bridge_reply_to_unowned_takes_control_then_delivers_on_its_row(self):
        ...
        doc = api._load(name)
        self.assertEqual((doc.origin, doc.purpose, doc.actor_user, doc.state), ("Human", "manual", self.users["one"], "Queued"))
        owner = frappe.db.get_value("CRM Conversation", self.doc.name, ["human_owner", "generation"], as_dict=True)
        self.assertEqual((owner.human_owner, owner.generation), (self.users["one"], doc.conversation_generation))
        self.assertTrue(frappe.db.exists("CRM Conversation Control Event",
    ...
    def test_outbox_bridge_refuses_what_ownership_forbids_without_writing(self):
        ...
        for state, reason in (("Bot", "bot_in_control"), ("Paused", "conversation_paused"), ("Closed", "conversation_closed")):
            frappe.db.set_value("CRM Conversation", self.doc.name, {"control_state": state, "human_owner": None})
        ...
        self.assertFalse(frappe.db.exists(api.DOCTYPE, {"transcript_message": ["is", "set"], "conversation": self.doc.name}))
    ...
    def test_outbox_bridge_closed_window_refuses_free_form_but_queues_a_template(self):
        ...
        frappe.set_user("Administrator")
        template = frappe.get_doc({"doctype": "WhatsApp Templates", "template_name": "fictional_notice_" + uuid4().hex[:6],
            "actual_name": "fictional_notice", "language_code": "es_MX", "category": "UTILITY",
            "template": "Fictional notice", "whatsapp_account": self.account.name, "status": "APPROVED",
            "creation": frappe.utils.now_datetime(), "modified": frappe.utils.now_datetime(), "owner": "Administrator"})
    ...
    def test_outbox_bridge_notice_ignores_owner_and_survives_a_control_change(self):
        ...
        row = self.row(text="Fictional repair notice", doco_sent_by_type="Automation",
                       doco_automation_source="tracker_notify:fictional", doco_actor_user=self.users["two"])
        name = self.intent_for(row)
        self.assertEqual((api._load(name).origin, api._load(name).actor_user), ("Automation", "Administrator"))
    ...
    def test_outbox_native_reply_without_a_row_is_projected_into_the_transcript(self):
        ...
        row = frappe.db.get_value("WhatsApp Message", bridge.PROJECTED_PREFIX + name,
            ["type", "to", "message", "status", "message_id", "whatsapp_account", *(["doco_sent_by_type"] if provenance else [])],
            as_dict=True)
        self.assertEqual((row.type, row.to, row.message, row.status, row.message_id, row.whatsapp_account),
                         ("Outgoing", self.peer, "Fictional Conversaciones reply", "Success", "wamid.projected", self.account.name))
        if provenance:
