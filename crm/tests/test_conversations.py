...
class TestConversations(IntegrationTestCase):
	...
	def setUpClass(cls):
		...
		for name in (api.DOCTYPE, api.EVENT, "WhatsApp Account", "Meta Webhook Receipt"):
			if not frappe.db.exists("DocType", name):
				raise RuntimeError(
					"Install controlled conversation schemas on the isolated lab first: " + name
				)
		...
		if frappe.db.has_column("WhatsApp Account", "doco_shop"):
			cls.shop = (
				frappe.get_doc({"doctype": "Social Shop", "shop_name": cls.prefix, "enabled": 1})
				.insert()
				.name
			)
	...
	def seed_bot(self):
		...
		frappe.db.set_value(
			api.DOCTYPE, self.doc.name, {"control_state": "Bot", "bot_enabled": 1, "human_owner": None}
		)
	...
	def observation(self, action="external_outbound", historical=False):
		return api.internal_apply_provider_event(
			"WhatsApp", self.account_id, self.peer, action, historical=historical
		)
	...
	def test_owner_transfer_manager_reason_and_release_never_bot(self):
		...
		self.assertEqual(
			(released["control_state"], released["bot_enabled"], released["generation"]), ("Human", 0, 4)
		)
	...
	def test_close_reopen_and_no_public_bot_activation(self):
		...
		self.assertEqual(
			(reopened["control_state"], reopened["bot_enabled"], reopened["generation"]), ("Human", 0, 4)
		)
	...
	def test_reference_permission_not_inherited_from_account(self):
		...
		inquiry = create_inquiry(
			{
				"title": "Fictional scoped conversation",
				"source_type": "Manual",
				"client_request_id": uuid4().hex,
			}
		)
		linked = api.get_or_create(
			"WhatsApp",
			self.account_id,
			self.peer + "1",
	...
	def test_current_shop_scope_and_target_owner(self):
		...
		frappe.db.delete(
			"User Permission", {"user": self.users["one"], "allow": "Social Shop", "for_value": self.shop}
		)
		with self.assertRaises(frappe.PermissionError):
			api.get_conversation(self.doc.name)
		with self.assertRaises(frappe.PermissionError):
	...
	def test_no_nested_transaction_boundary_or_sensitive_realtime(self):
		with (
			patch.object(frappe.db, "commit", side_effect=AssertionError("nested commit")),
	...
	def test_history_and_correlated_echo_never_change_authority(self):
		self.seed_bot()
		for kind, action, explicit in (
			("history", "external_outbound", False),
	...
	def test_provider_loss_and_return_never_resume_bot(self):
		...
		self.assertEqual(
			(returned["control_state"], returned["provider_control"], returned["bot_enabled"]),
			("Paused", "Ours", 0),
		)
	...
	def test_current_role_and_enabled_user_revocation(self):
		...
		self.command()
		frappe.db.delete(
			"Has Role", {"parent": self.users["one"], "parenttype": "User", "role": "Sales User"}
		)
		with self.assertRaises(frappe.PermissionError):
			self.command(generation=2)
		frappe.set_user(self.users["two"])
	...
	def test_account_rebinding_and_shop_movement_cannot_redirect(self):
		...
		for action in (
			lambda: api.get_conversation(self.doc.name),
	...
	def test_broker_list_never_exposes_another_reference(self):
		...
		inquiry = create_inquiry(
			{"title": "Fictional private inquiry", "source_type": "Manual", "client_request_id": uuid4().hex}
		)
		linked = api.get_or_create(
			"WhatsApp",
			self.account_id,
