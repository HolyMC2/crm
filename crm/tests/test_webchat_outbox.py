...
class TestWebchatOutbox(unittest.TestCase):
	def setUp(self):
		...
		self.enterContext(
			patch.object(frappe, "session", frappe._dict(user="Administrator", data=frappe._dict()))
		)
			...
			frappe.get_doc(
				{
					...
					"binding_key": webchat._key(profile, origin),
					"enabled": 1,
				}
			)
		).insert(ignore_permissions=True, set_name=self.account)
		created = now_datetime()
		session_name = secrets.token_hex(32)
		self.session = webchat._mark(
			frappe.get_doc(
				{
		...
		).insert(ignore_permissions=True, set_name=session_name)
	...
	def queue(self, text="Fictional support reply", request_id=None):
		return outbox.queue_message(
			self.conversation.name, 2, request_id or secrets.token_hex(16), {"type": "text", "text": text}
		)
	...
	def test_lost_local_commit_acknowledgement_preserves_accepted(self):
		...
		with (
			...
			patch.object(frappe.db, "commit", side_effect=RuntimeError("lost ack")),
		):
			outbox.dispatch_intent(name)
	...
	def test_staff_channel_assignment_is_current_and_explicit(self):
		user = frappe.get_doc(
			{
				"doctype": "User",
				"email": "webchat-" + secrets.token_hex(6) + "@example.invalid",
				"first_name": "Webchat operator",
		...
		with patch.object(frappe, "session", frappe._dict(user=user.name)):
			...
			self.assertEqual(
				threads.get_history(self.conversation.name)["conversation"]["name"], self.conversation.name
			)
		frappe.delete_doc("User Permission", permission.name, ignore_permissions=True)
		with (
			patch.object(frappe, "session", frappe._dict(user=user.name)),
