...
from crm.api import outbox_legacy as legacy
...
class LegacyGuardFixture(unittest.TestCase):
	def setUp(self):
		...
		self.payload = {
			...
			"to": self.peer,
			"type": "text",
			"text": {"body": "Fictional guarded reply"},
		}
		self.name = control.conversation_key("WhatsApp", self.account_id, self.peer)

	def guard(self, **kwargs):
		return legacy.guard_legacy_send(
			"WhatsApp",
			kwargs.get("account", self.account_id),
			kwargs.get("peer", self.peer),
	...
	def native_intent(self):
			...
			[
				{
					...
					"payload": {
						"change": {
							"value": {
								"messages": [{"from": self.peer, "timestamp": str(int(time.time()) - 10)}]
							}
						}
					},
				}
		...
		return outbox.queue_message(
			self.name, 2, uuid4().hex, {"type": "text", "text": self.payload["text"]["body"]}
		)["name"]
		...
		frappe.get_doc(
			{
				...
				"name": "legacy-private-" + uuid4().hex,
				"channel": "WhatsApp",
				"external_id": peer or self.peer,
				"role": "Staff",
				"enabled": 1,
				"owner": "Administrator",
...
class TestLegacyGuardSql(LegacyGuardFixture):
	...
	def test_any_control_state_denies_legacy_even_to_current_owner(self):
		...
		for state in ("Human", "Bot", "Paused", "Closed"):
			frappe.db.set_value(
				control.DOCTYPE, self.name, {"control_state": state, "human_owner": "Administrator"}
			)
			with (
				self.subTest(state=state),
				self.assertRaisesRegex(legacy.LegacySendBlocked, "^native_outbound_intent_required$"),
			):
				with self.guard():
	...
	def test_flags_roles_and_forged_context_do_not_grant_authority(self):
		self.open()
		with patch.object(
			frappe, "flags", frappe._dict(native_outbox=True, ignore_permissions=True, assistant_private=True)
		):
			with self.assertRaises(legacy.LegacySendBlocked):
				with self.guard():
		...
		for grant in (
			(
				...
				self.account_id,
				self.peer,
				outbox._canonical(self.payload).encode(),
				"fake",
			),
	...
	def test_missing_schema_and_current_read_error_are_static_holds(self):
		...
		with patch.object(
			frappe.db,
			"exists",
			side_effect=lambda dt, key, **kw: False if key == control.DOCTYPE else real_exists(dt, key, **kw),
		):
	...
	def test_http_failure_does_not_mask_exception_or_rollback_prior_writes(self):
		todo = frappe.get_doc(
			{"doctype": "ToDo", "description": "Preserved after HTTP double fails"}
		).insert()
	...
	def test_current_read_conflict_is_static_and_does_not_rollback_caller(self):
		with (
			patch.object(
				frappe.db, "get_value", side_effect=frappe.QueryDeadlockError("private SQL details")
			),
