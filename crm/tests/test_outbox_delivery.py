...
from crm.api import outbox_delivery as delivery
...
class TestOutboxDeliverySql(unittest.TestCase):
	def setUp(self):
		...
		for target in (
			...
			self.enterContext(
				patch(target, side_effect=AssertionError("delivery invoked outbound side effect"))
			)
		...
		return frappe.get_doc(
			{
				...
				"account_name": "delivery-" + uuid4().hex,
				...
				"app_secret": "fictional-delivery-secret",
			}
		).insert(ignore_permissions=True)

	def new_intent(self, mid=None, *, state="Accepted"):
		name = outbox.queue_message(
			self.conversation.name, 2, uuid4().hex, {"type": "text", "text": "Frozen native reply"}
		)["name"]
		with control.conversation_fence(self.conversation.name):
			...
			if state == "Accepted":
				outbox._transition(
					doc, "Accepted", provider_message_id=mid, accepted_at=delivery._timestamp(self.now - 9)
				)
			else:
	...
	def receipt(self, status="delivered", *, timestamp=None, mid=None, peer=None, account=None):
		...
		entry = {
			"id": mid or self.mid,
			"status": status,
			"recipient_id": peer or self.peer,
		...
		with (
			...
			patch.object(signature, "_accounts", return_value=[account]),
		):
	...
	def test_signed_guest_receipt_updates_native_intent_without_legacy_message(self):
		...
		self.assertFalse(
			frappe.db.exists(
				"WhatsApp Message", {"message_id": self.mid, "name": ["not like", "wa-native-%"]}
			)
		)
	...
	def test_delivered_then_read_uses_provider_times_and_does_not_downgrade(self):
		...
		self.intent.reload()
		self.assertEqual(
			(self.intent.state, self.intent.delivered_at, self.intent.read_at),
	...
	def test_foreign_account_or_peer_with_same_message_id_does_not_match(self):
		...
		self.assertFalse(
			frappe.db.exists(
				control.DOCTYPE, control.conversation_key("WhatsApp", self.account.phone_id, self.peer + "1")
			)
		)
	...
	def test_legacy_unkeyed_duplicate_targets_are_ambiguous_and_unchanged(self):
		...
		frappe.db.set_value(
			outbox.DOCTYPE, self.intent.name, "provider_message_key", None, update_modified=False
		)
	...
	def test_http_flag_only_nonprocessing_and_tampered_receipts_are_rejected(self):
		...
		with (
			...
			patch.object(frappe, "request", object()),
			self.assertRaises(delivery.DeliveryError),
		):
			delivery.apply_delivery_receipt(receipt.name)
	...
	def test_lost_human_authority_does_not_erase_provider_delivery(self):
		...
		with (
			patch.object(
				outbox, "_eligibility", side_effect=AssertionError("Delivery must not reauthorize a send")
			),
			patch.object(
				control,
				"_authorize",
				side_effect=AssertionError("Delivery must not require current actor rights"),
			),
		):
