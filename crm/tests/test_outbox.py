...
class TestOutbox(test_conversations.TestConversations):
	...
	def inbound(
		self, *, timestamp=None, account=None, peer=None, app="9800001", kind="message", state="Processed"
	):
		...
		name = record_events(
			[
				{
					"provider": "WhatsApp",
					"account_id": account or self.account_id,
					"app_id": app,
					"event_type": kind,
					"event_id": uuid4().hex,
					"payload": {
						"change": {
							"value": {"messages": [{"from": peer or self.peer, "timestamp": str(timestamp)}]}
						}
					},
				}
			]
	...
	def queue(self, text="Fictional manual reply", request_id=None, generation=2):
		return api.queue_message(
			self.doc.name, generation, request_id or uuid4().hex, {"type": "text", "text": text}
		)
	...
	def dispatch(self, name, result=None):
		with patch.object(
			api,
			"_gateway",
			return_value=result or {"state": "Accepted", "provider_message_id": "wamid.fictional"},
		) as send:
			self.worker(name)
	...
	def test_outbox_submitting_committed_before_exact_native_gateway(self):
		...
		with (
			patch.object(frappe.db, "commit", side_effect=checkpoint),
			patch.object(native_outbox, "send_frozen", side_effect=provider),
		):
			self.worker(name)
		...
		with self.assertRaises(frappe.PermissionError):
			api.require_dispatch(
				name, "WhatsApp", self.account_id, self.peer, payload=json.loads(doc.payload)
			)
	...
	def test_outbox_real_gateway_and_core_guard_share_exact_transport_body(self):
		...
		set_encrypted_password(
			"WhatsApp Account", self.account.name, "fictional-test-token", fieldname="token"
		)
		...
		body = api._canonical(
			{
				...
				"contacts": [{"input": self.peer, "wa_id": self.peer}],
				"messages": [{"id": "wamid.actual-chain-double"}],
			}
		).encode()
		...
		self.assertEqual(
			http.call_args.args[2], f"https://graph.facebook.com/v23.0/{self.account_id}/messages"
		)
	...
	def test_outbox_expired_submission_recovers_unknown_without_send(self):
		...
		frappe.db.set_value(
			...
			name,
			{
				...
				"state_log": api._canonical(
					[{"state": "Submitting", "at": str(now_datetime()), "reason": ""}]
				),
			},
		)
		doc, send = self.dispatch(name)
	...
	def test_outbox_current_claim_cannot_be_stolen(self):
		...
		frappe.db.set_value(
			api.DOCTYPE,
			name,
			{"state": "Claimed", "attempts": 1, "lease_until": now_datetime() + timedelta(minutes=1)},
		)
		doc, send = self.dispatch(name)
	...
	def test_outbox_unique_provider_key_and_synthetic_success_hold(self):
		...
		self.assertTrue(
			any(row.Column_name == "provider_message_key" and row.Non_unique == 0 for row in indexes)
		)
		doc, _ = self.dispatch(
			self.queue()["name"], {"state": "Accepted", "provider_message_id": "wamid.demo-fake"}
		)
		self.assertEqual(doc.state, "Unknown")
	...
	def test_outbox_explicit_suppression_blocks_service_reply(self):
		...
		frappe.get_doc(
			{
				"doctype": "Marketing Suppression",
				"party": "+52 1 555 010 0999",
				"party_type": "Phone",
				"channel": "All",
				"reason": "Manual",
			}
	...
	def test_outbox_automation_and_http_dispatch_remain_closed(self):
		...
		name = self.queue()["name"]
		with (
			patch.object(frappe.local, "request", object(), create=True),
