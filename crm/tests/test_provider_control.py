...
from crm.api import outbox, provider_control
...
def _require(*doctypes):
	...
	if not frappe.db.has_column("Messenger Page", "instagram_login_type"):
		raise unittest.SkipTest(
			"Run the doco_marketing migration for Messenger Page.instagram_login_type first."
		)
...
class _Isolated(unittest.TestCase):
	def setUp(self):
		...
		self.get = self.enterContext(
			patch.object(requests, "get", side_effect=lambda *a, **k: Response(payload=body()))
		)
		self.post = self.enterContext(
			patch.object(requests, "post", side_effect=AssertionError("No message sends"))
		)
			...
			frappe.get_doc(
				{
					...
					"page_name": "Provider Fictional " + uid[:8],
					"page_id": self.page_id,
					"ig_account_id": self.ig_id,
					"enabled": 1,
					"shop": self.shop,
					"page_access_token": self.token,
					"instagram_access_token": "fixture-ig-token-" + uid,
					"instagram_login_type": "Instagram Login",
				}
			)
	...
	def new_shop(self):
		return (
			frappe.get_doc(
				{"doctype": "Social Shop", "shop_name": self.prefix + "-" + uuid4().hex[:6], "enabled": 1}
			)
			.insert(ignore_permissions=True)
			.name
		)
	...
	def user(self, label, shop, *roles):
		...
		frappe.get_doc(
			{"doctype": "User Permission", "user": name, "allow": "Social Shop", "for_value": shop}
		).insert()
		return name
	...
	def current(self, conv=None):
		return frappe.db.get_value(
			control.DOCTYPE,
			(conv or self.conv).name,
			["control_state", "human_owner", "bot_enabled", "provider_control", "generation"],
			as_dict=True,
		)
	...
	def events(self, conv=None):
		return frappe.get_all(
			control.EVENT,
			filters={"conversation": (conv or self.conv).name, "action": "provider_verify"},
...
class TestVerify(_Isolated):
	def test_manager_query_records_ours_with_audit_and_sends_nothing(self):
		...
		self.assertEqual(
			(result["provider_control"], result["generation"], result["changed"], result["replayed"]),
			("Ours", 2, True, False),
		)
		...
		[event] = self.events()
		self.assertEqual(
			(event.origin, event.actor_user, event.from_generation, event.to_generation, event.reason),
	...
	def test_replay_returns_the_stored_answer_without_a_second_query(self):
		...
		self.assertEqual(
			(again["replayed"], again["provider_control"], again["generation"]), (True, "Ours", 2)
		)
	...
	def test_only_a_manager_or_the_current_owner_may_ask(self):
		...
		self.assertEqual(
			(result["provider_control"], result["human_owner"], result["generation"]),
			("Ours", self.seller, 3),
		)
		with self.assertRaises(frappe.PermissionError):
	...
	def test_other_app_keeps_the_human_owner_and_blocks_native_replies(self):
		...
		self.assertEqual(
			(result["verification"]["owner_kind"], result["verification"]["send_ready"]),
			("meta_inbox", False),
		)
	...
	def test_losing_control_revokes_the_bot_grant_and_returns_people(self):
		...
		with control.conversation_fence(self.conv.name), self.assertRaises(frappe.PermissionError):
			control.assert_current_generation(
				self.conv.name, 5, origin="Bot", actor_user=self.manager, run_name="fixture-run"
			)
		self.assert_nothing_sent()
	...
	def test_inconclusive_answers_never_grant_and_retire_a_stored_ours(self):
		...
		cases = [
			...
			(
				...
				lambda: R(
					302,
					headers={"Location": "https://evil.invalid", "Content-Type": "application/json"},
					payload={},
				),
			),
		...
		for outcome, factory in cases:
			...
			self.assertEqual(
				...
				(outcome, "Unknown", True, generation + 1),
				outcome,
			)
			again = self.verify(self.manager)
			self.assertEqual(
				(again["provider_control"], again["changed"], again["generation"]),
				("Unknown", False, generation + 1),
				outcome,
			)
	...
	def test_instagram_login_is_unsupported_without_a_query_and_never_ours(self):
		...
		self.assertEqual(
			(result["verification"]["outcome"], result["provider_control"], result["changed"]),
	...
	def test_instagram_facebook_login_uses_the_documented_page_query(self):
		...
		args, kwargs = self.get.call_args
		self.assertEqual(
			(args, kwargs["params"], kwargs["headers"]["Authorization"]),
			(
				(f"https://graph.facebook.com/v25.0/{self.page_id}/thread_owner",),
				{"recipient": igsid},
				"Bearer " + self.token,
			),
		)
	...
	def test_missing_app_identity_blocks_without_a_query(self):
		...
		result = self.verify(self.manager)
		self.assertEqual(
			(result["verification"]["outcome"], result["provider_control"]), ("unconfigured", "Unknown")
		)
	...
	def test_account_token_or_app_rotation_during_the_query_invalidates_the_answer(self):
		...
		for change in (
			lambda: set_encrypted_password(
				"Messenger Page", self.page, "rotated-" + uuid4().hex, "page_access_token"
			),
		...
		self.get.side_effect = rotating(
			lambda: frappe.db.set_value("Messenger Page", self.page, "shop", self.new_shop())
		)
		with self.assertRaises(frappe.PermissionError):
			self.verify(self.manager)
...
class TestFinalDispatch(_Isolated):
	...
	def queue(self, generation):
		with self.as_user(self.seller):
			return outbox.queue_message(
				...
				generation,
				"reply-" + uuid4().hex,
				{"type": "text", "text": "Tu equipo está listo"},
			)["name"]
	...
	def test_final_send_rechecks_the_owner_and_denial_never_posts(self):
		...
		state = frappe.db.get_value(
			outbox.DOCTYPE, stale, ["state", "reason_code", "provider_message_id"], as_dict=True
		)
		self.assertEqual(
			(state.state, state.reason_code, state.provider_message_id),
		...
		self.assertEqual(
			frappe.db.get_value(outbox.DOCTYPE, current, ["state", "provider_message_id"]), ("Accepted", mid)
		)
		self.assertEqual(
			[call[0] for call in calls.mock_calls if call[0] in {"get", "post"}], ["get", "post"]
		)
		self.assertEqual(
			self.post.call_args.args, (f"https://graph.facebook.com/v25.0/{self.page_id}/messages",)
		)
