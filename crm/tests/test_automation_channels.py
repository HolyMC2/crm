...
class TestDepartmentQueues(_Isolated):
    def setUp(self):
        ...
        if frappe.db.has_column("WhatsApp Account", "doco_shop"):
            self.shop = frappe.get_doc({"doctype": "Social Shop", "shop_name": self.prefix, "enabled": 1}).insert().name
            for user in (self.sales, self.support):
                frappe.get_doc({"doctype": "User Permission", "user": user, "allow": "Social Shop",
                                "for_value": self.shop}).insert()
        self.account_id = "97" + str(int(uuid4().hex[:12], 16))
        frappe.get_doc({"doctype": "WhatsApp Account", "account_name": self.prefix, "phone_id": self.account_id,
                        "status": "Active", "mode": "Demo", "doco_shop": self.shop, "is_default_incoming": 0,
                        "is_default_outgoing": 0}).insert()
    ...
    def test_routing_grants_department_eligibility_without_sales_roles(self):
        ...
        with self.as_user(self.support):
            ...
            self.assertEqual((item["state"], item["next_action"], item["department"]), ("Mine", "reply", "support"))
    ...
    def test_owned_conversation_routes_only_by_owner_or_manager(self):
        ...
        if self.shop:
            frappe.get_doc({"doctype": "User Permission", "user": second, "allow": "Social Shop",
                            "for_value": self.shop}).insert()
    ...
    def test_department_record_link_replaces_unreadable_sales_reference(self):
        ...
        inquiry = create_inquiry({"title": "Fictional inquiry " + self.prefix, "source_type": "Manual", "people": [],
                                  "client_request_id": self.prefix})
        referenced = control.get_or_create("WhatsApp", self.account_id, "5215550100903",
        ...
        issue = frappe.get_doc({"doctype": "Issue", "subject": "Fictional support case " + self.prefix}).insert()
...
class TestInboundPolicy(_Isolated):
    def setUp(self):
        ...
        frappe.get_doc({"doctype": "WhatsApp Account", "account_name": self.prefix, "phone_id": self.account_id,
                        "status": "Active", "mode": "Demo", "is_default_incoming": 0, "is_default_outgoing": 0}).insert()
    ...
    def test_policy_is_service_owned_reviewed_and_manager_only(self):
        with self.assertRaises(frappe.PermissionError):
            frappe.get_doc({"doctype": policies.DOCTYPE, "title": "Forged", "provider": "WhatsApp",
                            "account_id": self.account_id, "flow": "x", "status": "Published", "revision": 1,
                            "cooldown_minutes": 0}).insert(ignore_permissions=True)
        policy = policies.save_policy({"title": "Fictional inbound", "provider": "WhatsApp", "account_id": self.account_id,
                                       "flow": "Fictional missing flow", "department": "support"})
        ...
        with self.as_user(self.user("seller", "Sales User")), self.assertRaises(frappe.PermissionError):
            policies.save_policy({"title": "Seller policy", "provider": "WhatsApp", "account_id": self.account_id,
                                  "flow": "x"})
    ...
    def test_policy_grant_requires_trusted_ingress_and_published_policy(self):
        for command in ("manual:1", "inbound:meta:" + "a" * 64, "inbound:webchat:missing"):
            with self.subTest(command=command), self.assertRaises(frappe.PermissionError):
                control.begin_policy_bot(self.doc.name, 1, command, run_name="fictional-run", policy_name="fictional")
    ...
    def test_customer_message_without_claimed_receipt_is_refused(self):
        with self.assertRaises(frappe.PermissionError):
            control.internal_customer_message("WhatsApp", self.account_id, "5215550100911",
                                              receipt_name="b" * 64, text="hola")
...
class TestWebchatContinuation(_Isolated):
    ...
    def setUp(self):
        ...
        self.channel = webchat.configure_channel(label="Fictional Webchat", profile="fictional-" + uuid4().hex,
                                                 public_origin="https://fictional.example.invalid", enabled=1)
        ...
        frappe.db.set_value(control.DOCTYPE, self.name, {"control_state": "Bot", "bot_enabled": 1, "human_owner": None},
    ...
    def request(self, body, endpoint, capability=None):
        ...
        request = Request(EnvironBuilder(path="/api/method/crm.api.webchat." + endpoint, method="POST",
            headers={"Authorization": "Bearer " + capability} if capability else {},
            data=json.dumps(body).encode(), content_type="application/json",
            environ_base={"REMOTE_ADDR": "192.0.2.89"}).get_environ())
    ...
    def test_unmatched_reply_keeps_human_fallback_and_routes_flow_department(self):
        self.adapter.accept_customer_input = lambda doc, evidence: {
            "accepted": False, "reason_code": "customer_requested_person", "department": "support"}
        ...
        self.assertEqual((doc.control_state, doc.bot_enabled, doc.department, doc.automation_state),
                         ("Human", 0, "support", "Handed off"))
        self.assertEqual(doc.generation, generation + 1)
        events = frappe.get_all(control.EVENT, filters={"conversation": self.name, "action": "customer_reply"},
                                fields=["reason", "result_json"], order_by="creation desc", limit=1)
    ...
    def test_accepted_input_keeps_bot_only_for_the_run_holding_the_grant(self):
        ...
        frappe.db.set_value(control.DOCTYPE, self.name, {"control_state": "Bot", "bot_enabled": 1}, update_modified=False)
        generation = control._load(self.name).generation
    ...
    def test_policy_start_failure_never_loses_the_customer_message(self):
        frappe.db.set_value(control.DOCTYPE, self.name, {"control_state": "Human", "bot_enabled": 0}, update_modified=False)
        self.adapter.start_policy_run = lambda *args: (_ for _ in ()).throw(RuntimeError("fictional start failure"))
        fake = SimpleNamespace(name="fictional-policy")
        with patch.object(policies, "published_for", return_value=fake), \
...
class TestHookedChannels(_Isolated):
    def test_hooked_transport_requires_exactly_one_installed_adapter(self):
        ...
        with patch.object(frappe, "get_hooks", return_value={"Messenger": ["frappe.utils", "frappe.utils.data"]}):
            self.assertIsNone(outbox.channel_adapter("Messenger"))
