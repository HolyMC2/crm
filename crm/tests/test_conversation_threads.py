...
class TestConversationThreads(IntegrationTestCase):
	...
	def setUpClass(cls):
		...
		cls.shop = (
			frappe.get_doc({"doctype": "Social Shop", "shop_name": cls.prefix, "enabled": 1}).insert().name
		)
		frappe.get_doc(
			{"doctype": "User Permission", "user": cls.actor, "allow": "Social Shop", "for_value": cls.shop}
		).insert()
		...
		self.account = frappe.get_doc(
			{
				...
				"account_name": self.prefix + uuid4().hex[:6],
	...
	def inquiry(self):
		name = uuid4().hex
		frappe.get_doc(
			{"doctype": "CRM Inquiry", "name": name, "title": "Fictional inquiry", "status": "New"}
		).db_insert()
		return name
	...
	def test_account_metadata_and_explicit_legacy_open(self):
		...
		self.assertEqual(
			(opened["control_state"], opened["bot_enabled"], opened["generation"]), ("Human", 0, 1)
		)
	...
	def test_reference_list_preserves_exact_identity_without_materializing(self):
		...
		doc = control.get_or_create(
			"WhatsApp", self.account_id, self.peer, reference_doctype="CRM Inquiry", reference_name=inquiry
		)
		other = self.inquiry()
	...
	def test_reference_list_implies_threads_from_the_record_own_messages(self):
		...
		self.assertEqual(
			[(i["name"], i["peer_id"], i["materialized"], i["preview"]) for i in items],
	...
	def test_exact_account_peer_no_phone_suffix_or_default_fallback(self):
		...
		self.assertEqual(
			[r["peer_id"] for r in api.list_threads("WhatsApp", self.account_id)["items"]], ["5215550198766"]
		)
	...
	def test_whatsapp_reply_spelling_joins_the_customer_thread(self):
		...
		self.message(
			...
			template=template,
			creation="2026-01-01 12:05:00",
			template_parameters=json.dumps(["Pablo", "RO-1"]),
			**{"from": None},
		)
		...
		history = api.get_history(opened["name"])
		self.assertEqual(
			[(m["direction"], m["content"]) for m in history["messages"]],
			[("in", "Hola, ¿ya quedó?"), ("out", "Listo Pablo, folio RO-1")],
		)
		frappe.get_doc(
			{
				"doctype": "Contact",
				"name": "contact_" + uuid4().hex[:8],
				"first_name": "Pablo",
				"full_name": "Pablo Hernández",
				"mobile_no": "+52 1 555 019 8766",
			}
		).db_insert()
		self.assertEqual(api.get_history(opened["name"])["conversation"]["display_name"], "Pablo Hernández")
		self.assertEqual(
			api.list_threads("WhatsApp", self.account_id)["items"][0]["display_name"], "Pablo Hernández"
		)
	...
	def test_denied_sources_never_supply_preview_body_attachment_or_count(self):
		...
		self.message(
			...
			reference_name="private-chart",
			attach="https://private.invalid/chart",
			creation="2026-03-01 12:00:00",
		)
		for i in range(205):
			self.message(
				"DENIED SECRET",
	...
	def test_private_disabled_principal_and_historical_assistant_are_excluded(self):
		...
		frappe.get_doc(
			{"doctype": "Books Chat Log", "name": uuid4().hex, "chat_id": "wa:" + history_peer}
		).db_insert()
		with self.assertRaises(frappe.PermissionError):
	...
	def test_denied_core_reference_never_reappears_as_legacy_thread(self):
		...
		frappe.db.set_value(
			control.DOCTYPE, doc.name, {"reference_doctype": "Patient", "reference_name": "private-chart"}
		)
		self.assertEqual(api.list_threads("WhatsApp", self.account_id)["items"], [])
		with self.assertRaises(frappe.PermissionError):
			api.open_thread("WhatsApp", self.account_id, self.peer)
	...
	def test_remote_attachment_is_withheld_and_html_stays_plain_text(self):
		self.message(
			'<img src=x onerror="alert(1)">', attach="https://provider.invalid/secret", content_type="image"
		)
		opened = api.open_thread("WhatsApp", self.account_id, self.peer)
	...
	def test_messenger_and_instagram_keep_exact_page_platform_and_peer(self):
		...
		for provider, identity in (("Messenger", page_id), ("Instagram", ig_id)):
			with self.subTest(provider=provider):
				...
				self.assertEqual(
					[m["content"] for m in api.get_history(opened["name"])["messages"]], [provider + " exact"]
				)
