...
class CRMConversation(Document):
	...
	def has_permission(self, permtype="read", *, debug=False, user=None):
		...
		return (
			permtype in {"create", "write"} and self.flags.get("crm_conversation_service") is _SERVICE_TOKEN
		)
	...
	def validate(self):
		from crm.api.conversations import _SERVICE_TOKEN, STATES, conversation_key
		...
		if not self.is_new():
			previous = frappe.db.get_value(
				self.doctype,
				self.name,
				["provider", "account_id", "peer_id", "identity_version", "generation"],
				as_dict=True,
				for_update=True,
			)
			if any(
				self.get(f) != previous.get(f)
				for f in ("provider", "account_id", "peer_id", "identity_version")
			):
				frappe.throw(frappe._("Conversation identity is immutable."))
