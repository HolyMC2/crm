...
def race(action):
	...
	assert all(
		e[0] == "QueryDeadlockError" or e == ("ValidationError", "QueryDeadlockError") for e in errors
	), errors
...
try:
	with (
		...
		frappe.db.commit()
		print(
			"PASS concurrent known-source capture: one snapshot across retries; safe conflicts="
			+ str(conflicts)
		)
		...
		frappe.get_doc(
			{
				"doctype": "User",
				"email": USER,
				"first_name": "Fictional revoked agent",
				"send_welcome_email": 0,
				"roles": [{"role": "Sales User"}],
			}
		).insert()
		revoked = api.create_inquiry(
			{
				"title": KEY + "-revoked",
				"client_request_id": KEY + "-revoked",
		...
		assert outcomes == ["permission_denied"] or outcomes == [
			("ValidationError", "QueryDeadlockError")
		], outcomes
		frappe.set_user(USER)
		try:
			api.convert_person(revoked["name"], revoked["people"][0]["person_key"])
		...
		print(
			"PASS revoked assignee: stale snapshot cannot convert, fresh retry denied; outcomes="
			+ str(outcomes)
		)
finally:
	frappe.set_user("Administrator")
	...
	names = frappe.get_all(
		"CRM Inquiry", filters={"title": ["in", [KEY, KEY + "-revoked", KEY + "-source"]]}, pluck="name"
	)
	leads = frappe.get_all(
		"CRM Lead", filters={"first_name": ["in", [KEY, KEY + "-revoked", KEY + "-source"]]}, pluck="name"
	)
	for name in names:
