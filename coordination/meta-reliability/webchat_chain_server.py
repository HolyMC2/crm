...
def fixed_init(site=None, *a, **kw):
	...
	assert all(
		int(frappe.conf.get(key) or 0) == 1 for key in ("maintenance_mode", "pause_scheduler", "mute_emails")
	)
...
def snapshot(data):
	...
	return {
		...
		"messages": frappe.get_all(
			webchat.MESSAGE,
			filters={"channel": data["channel_id"]},
		...
		"persistent_holds": [
			int(frappe.conf.get(k) or 0) for k in ("maintenance_mode", "pause_scheduler", "mute_emails")
		],
		"blocked": counts,
	}
