		...
		with blocked():
			...
			if mode == "queue":
				...
				result = control.apply_control(
					cfg["conversation"], "transfer", cfg["generation"], cfg["request"], owner=cfg["target"]
				)
				frappe.db.commit()
				...
				frappe.flags.crm_outbox_dispatch = {
					"intent": doc.name,
					"provider": "Webchat",
				...
				with (
					patch.object(webchat, "deliver_local", side_effect=deliver),
...
def start(mode, cfg, *, go=True):
	proc = subprocess.Popen(
		[sys.executable, str(Path(__file__).resolve()), mode, json.dumps(cfg)],
...
def durable(intent):
	...
	row = frappe.db.get_value(
		outbox.DOCTYPE,
		intent,
		["state", "attempts", "provider_message_id", "reason_code", "conversation_generation"],
		as_dict=True,
	)
...
def queued(cfg, label):
	...
	result = outbox.queue_message(
		cfg["conversation"], cfg["generation"], label, {"type": "text", "text": text(label)}
	)
	frappe.db.commit()
	return result["name"]
...
def fixture():
	...
	session = webchat._mark(
		frappe.get_doc(
			{
				"doctype": webchat.SESSION,
				"name": session_name,
				"channel": channel["account_id"],
				"peer_id": peer,
	...
	frappe.get_doc(
		{
			"doctype": "User",
			"email": target,
			"first_name": "Fictional Webchat process proof",
			"enabled": 1,
...
def cleanup(cfg):
	...
	for name in frappe.get_all(
		outbox.DOCTYPE,
		filters={"conversation": current.name, "state": ["in", ["Queued", "Blocked", "Deferred", "Failed"]]},
		pluck="name",
	):
	...
	webchat.configure_channel(
		channel.label,
		channel.profile,
		channel.public_origin,
		enabled=0,
		channel_id=channel.name,
		expected_modified=str(channel.modified),
	)
	...
	assert (
		frappe.db.count(
			outbox.DOCTYPE,
			{"conversation": current.name, "state": ["in", ["Queued", "Claimed", "Submitting"]]},
		)
		== 0
	)
	emit(
		...
		with blocked():
			try:
				...
				if skip_race:
					...
					emit(
						"same_request_race_not_run",
						reason="Independently continue remaining scenarios after retained race failure",
					)
				else:
				...
				crash.kill()
				finish(crash, -signal.SIGKILL)
				assert durable(before) == observed
				recovery = start("dispatch", {**cfg, "intent": before})
				finish(recovery)
				row = durable(before)
				assert (row["state"], row["attempts"], row["messages"]) == ("Accepted", 1, 1)
				...
				crash.kill()
				finish(crash, -signal.SIGKILL)
				replay = start("dispatch", {**cfg, "intent": after})
				finish(replay)
				...
				assert line(attacker)["event"] == "forgery_denied"
				finish(attacker)
				row = durable(forged)
				...
				assert (
					frappe.db.count(
						outbox.DOCTYPE, {"conversation": cfg["conversation"], "state": "Submitting"}
					)
					== 0
				)
				emit(
					"PARTIAL_PASS" if skip_race else "PASS",
					scenarios=5 if skip_race else 6,
					real_commits=True,
					sigkill_checkpoints=2,
