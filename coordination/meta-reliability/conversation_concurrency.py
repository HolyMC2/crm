...
def blocked():
	...
	with ExitStack() as stack:
		...
		stack.enter_context(
			__import__("unittest.mock", fromlist=["patch"]).patch("frappe.enqueue", side_effect=queue)
		)
	...
	try:
		with blocked():
			...
			if mode in {"race", "take"}:
				try:
					result = api.apply_control(
						cfg["name"], "take", cfg["generation"], cfg["command"], reason=cfg.get("reason")
					)
					frappe.db.commit()
				...
				with api.conversation_fence(cfg["name"]):
					...
					api._persist_transition(
						...
						origin="System",
						actor=cfg["actor"],
						action="proof_dispatch_submitting",
...
def fixture():
	...
	for index in (1, 2):
		...
		frappe.get_doc(
			{
				"doctype": "User",
				"email": user,
				"first_name": "Fictional control proof",
				"enabled": 1,
		...
		if shop:
			frappe.get_doc(
				{"doctype": "User Permission", "user": user, "allow": "Social Shop", "for_value": shop}
			).insert()
...
def cleanup(cfg):
	...
	if doc.control_state != "Closed":
		api.apply_control(
			doc.name, "close", doc.generation, cfg["prefix"] + "-close", reason="Fictional proof completed"
		)
	...
	frappe.db.commit()
	remaining = {
		"accounts": frappe.db.count("WhatsApp Account", {"name": cfg["account"]}),
	...
	emit(
		"cleanup",
		retained="One closed private fictional conversation plus immutable control events; no account/users/shop remain",
		**remaining,
	)
...
def proof():
	...
	try:
		with blocked():
			cfg = fixture()
			commands = [
				dict(name=cfg["name"], actor=user, generation=1, command=cfg["prefix"] + "-" + str(i))
				for i, user in enumerate(cfg["users"])
			]
			racers = [spawn("race", command) for command in commands]
			...
			holder = spawn("hold", hold_cfg)
			children.append(holder)
			assert line(holder)["event"] == "ready"
			assert line(holder)["event"] == "submitting_committed_fence_held"
			denied = spawn("stale", hold_cfg)
			children.append(denied)
			assert line(denied)["event"] == "ready"
			assert line(denied)["event"] == "dispatch_denied"
			finish(denied)
			takeover_cfg = dict(
				name=cfg["name"],
				generation=2,
				actor="Administrator",
				command=cfg["prefix"] + "-takeover",
				reason="Fictional fence takeover",
			)
			takeover = spawn("take", takeover_cfg)
			children.append(takeover)
