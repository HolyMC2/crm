...
def row(name):
	...
	return frappe.db.get_value(
		outbox.DOCTYPE, name, ["state", "attempts", "provider_message_id", "reason_code"], as_dict=True
	)
		...
		if mode == "receipt":
			...
			record_events(
				[
					{
						"provider": "WhatsApp",
						"account_id": "9700000000999",
						"app_id": "9700001",
						"event_type": "proof_unrelated",
						"event_id": uuid4().hex,
			...
			result = outbox.queue_message(
				...
				cfg["generation"],
				cfg["request"],
				{"type": "text", "text": "Fictional process proof"},
			)
			frappe.db.commit()
			...
			def provider(intent, payload):
				outbox.require_dispatch(
					intent.name, intent.provider, intent.account_id, intent.peer_id, payload=payload
				)
	...
	with base.blocked(), patch.object(outbox, "_enqueue"):
		try:
			...
			frappe.db.set_value(
				"WhatsApp Account",
				cfg["account"],
				{"mode": "Live", "app_id": "9700001", "business_id": "9700002"},
			)
			current = control._load(cfg["name"])
			from frappe_whatsapp.webhook_receipts import record_events

			receipt = record_events(
				[
					{
						"provider": "WhatsApp",
						...
						"app_id": "9700001",
						"event_type": "message",
						"event_id": "proof-" + uuid4().hex,
						"payload": {
							"change": {
								"value": {
									"messages": [
										{"from": current.peer_id, "timestamp": str(int(time.time()) - 10)}
									]
								}
							}
						},
			...
			run = {
				...
				"request": "same-browser-request",
				"attempt_file": "/tmp/p5-attempts-" + uuid4().hex,
			}
			...
			stale = spawn("dispatch", {**run, "intent": pending})
			finish(stale)
			assert row(pending).state == "Cancelled"
			assert Path(run["attempt_file"]).read_text().splitlines() == [run["intent"]]
			emit(
				"race_and_takeover_pass",
				unique_intents=2,
				provider_double_attempts=1,
				unrelated_receipt_unblocked=True,
			)
			...
			frappe.db.set_value(
				outbox.DOCTYPE, crash_intent, "lease_until", now_datetime() - timedelta(seconds=1)
			)
			frappe.db.commit()
			recovery = spawn("dispatch", {**run, "intent": crash_intent})
			finish(recovery)
			again = spawn("dispatch", {**run, "intent": crash_intent})
			finish(again)
			...
			frappe.db.set_value(
				outbox.DOCTYPE, claim_intent, "lease_until", now_datetime() - timedelta(seconds=1)
			)
			frappe.db.commit()
			recovery = spawn("dispatch", {**run, "intent": claim_intent})
			finish(recovery)
			...
			if cfg:
				...
				if current.control_state != "Closed":
					control.apply_control(
						...
						current.generation,
						"proof-cleanup",
						reason="Fictional outbox proof complete",
					)
				...
				emit(
					...
					fictional_accounts_users_disabled=True,
					audit_retained=True,
				)
			frappe.destroy()
