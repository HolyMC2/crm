...
expected = (
	{"frappe", "crm"}
	if args.site == "core"
	else {"frappe", "erpnext", "crm", "doco", "doco_marketing", "frappe_whatsapp", "mercado", "scanner_kit"}
)
...
print(
	json.dumps({"site": SITE, "apps": paths, "persistent_flags": [1, 1, 1], "page_exists": True}), flush=True
)
...
if args.mode == "setup":
	...
	frappe.get_doc(
		{
			"doctype": "User",
			"email": actor,
			"first_name": "Fictional Desk operator",
			"enabled": 1,
	...
	if args.site == "eight":
		...
		denied_shop = frappe.get_doc(
			{"doctype": "Social Shop", "shop_name": tag + " denied", "enabled": 1}
		).insert()
		frappe.get_doc(
			{"doctype": "User Permission", "user": actor, "allow": "Social Shop", "for_value": shop.name}
		).insert()
		...
		denied_account = frappe.get_doc(
			{
				"doctype": "WhatsApp Account",
				"account_name": tag + " denied",
				"phone_id": account_id + "9",
				"status": "Active",
				"mode": "Demo",
				"doco_shop": denied_shop.name,
				"is_default_incoming": 0,
				"is_default_outgoing": 0,
			}
		).insert()
		...
		samples = [
			(
				peers[0],
				account.name,
				'Fictional customer <img src=x onerror="window.fixtureAttack=1"> ' + "LongWord" * 30,
				{},
			),
			(
				peers[0],
				account.name,
				"CLINICAL_MIXED_SECRET",
				{"reference_doctype": "Patient", "reference_name": tag},
			),
			(peers[1], account.name, "PRIVATE_ASSISTANT_SECRET", {}),
			(
				peers[2],
				account.name,
				"CLINICAL_ONLY_SECRET",
				{"reference_doctype": "Patient", "reference_name": tag},
			),
		...
		data.update(
			...
			denied_account=denied_account.name,
			denied_account_id=account_id + "9",
			shop=shop.name,
			denied_shop=denied_shop.name,
			peer=peers[0],
			private_peer=peers[1],
			clinical_peer=peers[2],
	...
	print(
		json.dumps(
			{"fixture": str(FIXTURE), "actor": actor, "fictional_only": True, "blocked_calls": blocked_calls}
		),
		flush=True,
	)
	...
	result = {
		"site": SITE,
		"actor": data["actor"],
		"user_enabled": frappe.db.get_value("User", data["actor"], "enabled"),
	}
	if args.site == "eight":
		...
		result.update(
			...
			conversation_state=frappe.db.get_value(
				control.DOCTYPE, name, ["control_state", "generation", "bot_enabled"], as_dict=True
			),
			accounts=[
				frappe.db.get_value("WhatsApp Account", value, ["name", "status", "mode"], as_dict=True)
				for value in (data["account"], data["denied_account"])
			],
			shops=[
				frappe.db.get_value("Social Shop", value, ["name", "enabled"], as_dict=True)
	...
	if args.mode == "cleanup":
		...
		frappe.db.commit()
		result["cleanup"] = (
			"Audit and fictional messages retained; conversations closed, users/accounts/shops disabled"
		)
		...
		json.dumps(
			{
				"serving": args.port,
				"fixed_site": SITE,
				"transport": "requests/SMTP blocked; sendmail/enqueue suppressed",
			}
		),
		flush=True,
	)
