<template>
  <details class="border-b border-outline-gray-1 bg-surface-base px-5 py-3">
    ...
    <p class="mt-2 text-sm text-ink-gray-6">
      {{
        __(
          'Las notificaciones de grupos de Facebook no garantizan un evento disponible para CRM. Puedes capturar una consulta manualmente aunque la conexión falle.',
        )
      }}
    </p>
    ...
    <p v-if="error" class="mt-2 text-sm text-ink-red-7" role="alert">
      {{ error }}
    </p>
    <p
      ...
      >
        <div class="flex flex-wrap items-center gap-2">
          ...
          <span class="text-ink-gray-6">{{
            __(
              captureHealthLabel(
                verified[connection.page_id]?.state || connection.state,
              ),
            )
          }}</span>
          <button
          ...
          >
            {{
              checking === connection.page_id
                ? __('Verificando…')
                : __('Verificar en Meta')
            }}
          </button>
        </div>
        ...
        <p
          v-if="verified[connection.page_id]?.checked_at"
    ...
    <button
      v-if="error"
      type="button"
      class="mt-2 text-sm text-ink-blue-9"
      @click="reload"
    >
...
import {
  captureHealthLabel,
  newCaptureHealthRequests,
} from '@/utils/socialCaptureHealth'
...
async function reload() {
  ...
  try {
    const result = await call(
      'doco_marketing.services.social.capture_health.get_capture_health',
    ...
    if (requests.current(request)) {
      error.value = __(
        'El diagnóstico no está disponible. Revisa el acceso y la versión de la integración. La captura manual sigue disponible.',
      )
    }
  } finally {
...
async function verify(pageId) {
  ...
  try {
    const result = await call(
      'doco_marketing.services.social.capture_health.inspect_page',
      { page_id: pageId },
    )
    ...
    if (requests.current(request))
      error.value = __(
        'No se pudo verificar la página. Revisa el acceso e intenta de nuevo.',
      )
  } finally {
