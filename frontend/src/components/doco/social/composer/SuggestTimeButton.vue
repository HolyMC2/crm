...
<template>
  <div class="relative">
    ...
    <div
      v-if="open"
      class="absolute right-0 z-20 mt-1 w-[280px] rounded-lg border border-outline-gray-2 bg-surface-base p-2 shadow-xl"
    >
      <div v-if="loading" class="px-1 py-2 text-[12px] text-ink-gray-5">
        {{ __('Buscando el mejor horario…') }}
      </div>
      <div
        v-else-if="!suggestions.length"
        class="px-1 py-2 text-[12px] text-ink-gray-4"
      >
        {{ __('Sin datos suficientes todavía.') }}
      </div>
      <template v-else>
        <button
          ...
          @click="choose(s)"
        >
          <span class="text-[12px] font-semibold text-ink-gray-8">{{
            label(s)
          }}</span>
          <span v-if="s.reason" class="block text-[10.5px] text-ink-gray-5">{{
            s.reason
          }}</span>
        </button>
      </template>
      <p
        v-if="fbOnly"
        class="mt-0.5 border-t border-outline-gray-1 px-1 pt-1.5 text-[10px] text-ink-gray-4"
      >
        {{
          __(
            'Solo datos de Facebook (Instagram aún no tiene publicaciones para medir).',
          )
        }}
      </p>
    </div>
