<!--
  ...
  per-photo alt-text list (accesibilidad; save_post round-trips media alt_text). It emits
  replacement media arrays through v-model so the parent form and preview stay current
  without mutating a prop. Read-only once the post is live (canCancel=false).
-->
<template>
  <template v-if="media.length || canCancel">
    ...
    <div class="mb-1 flex flex-wrap gap-2">
      <div
        ...
        :class="
          mediaOver === i && mediaDrag !== i
            ? 'border-green-500 dark:border-green-400 ring-2 ring-green-300 dark:ring-green-500'
      ...
      >
        ...
        <span
          v-if="media.length > 1"
          class="absolute bottom-0 left-0 rounded-tr bg-black/50 px-1 text-[9px] font-bold text-white"
          >{{ i + 1 }}</span
        >
        <button
          v-if="canCancel"
      ...
      <button
        v-if="canCancel && media.length < 10"
        type="button"
        class="flex size-16 flex-col items-center justify-center gap-0.5 rounded-md border border-dashed border-outline-gray-3 text-ink-gray-5 hover:border-green-500 dark:hover:border-green-400 hover:text-ink-green-7 disabled:opacity-50"
        :disabled="uploadBusy > 0"
        :title="__('Agregar fotos')"
        @click="pickMedia"
      >
        <span v-if="uploadBusy" class="px-1 text-[9.5px] font-semibold">{{
          __('subiendo…')
        }}</span>
        <template v-else>
    ...
    <p class="mb-2 text-[10.5px] text-ink-gray-4">
      {{
        __(
          'Hasta 10 fotos (JPG/PNG). Varias fotos se publican como carrusel en FB.',
        )
      }}
    </p>
    <input
    ...
    <template v-if="media.length">
      <label class="mb-0.5 block text-[11px] font-semibold text-ink-gray-6">{{
        __('Texto alternativo')
      }}</label>
      <p class="mb-1.5 text-[10.5px] text-ink-gray-4">
        {{ __('Accesibilidad — describe la imagen.') }}
      </p>
      <div
        v-for="(m, i) in media"
        :key="'alt-' + m.media_file + ':' + i"
        class="mb-1.5 flex items-center gap-2"
      >
        <img
          :src="m.media_file"
          class="size-8 flex-none rounded object-cover"
          alt=""
        />
        <input
          :value="m.alt_text"
          @input="updateAlt(i, $event.target.value)"
          type="text"
          :disabled="!canCancel"
          ...
          :placeholder="
            __('Foto') + ' ' + (i + 1) + ' — ' + __('describe la imagen…')
          "
        />
      </div>
    </template>
...
const media = defineModel('media', { type: Array, default: () => [] })
...
async function onMediaPick(e) {
  ...
  const room = 10 - media.value.length
  ...
  for (const f of files.slice(0, Math.max(room, 0))) {
    ...
    try {
      ...
      media.value = [
        ...media.value,
        { media_file: up.file_url, media_type: 'Image', alt_text: '' },
      ]
    } catch {
...
function removeMedia(i) {
  media.value = media.value.filter((_, index) => index !== i)
}
function updateAlt(i, value) {
  media.value = media.value.map((row, index) =>
    index === i ? { ...row, alt_text: value } : row,
  )
}
