...
<template>
  ...
  <div v-if="isList" class="p-3">
    ...
    <div
      ...
      :key="day.key"
      class="mb-2 rounded-lg border border-outline-gray-2 bg-surface-base p-2"
    >
      <div class="mb-1 flex items-center justify-between">
        <span
          class="text-[11.5px] font-bold"
          :class="day.isToday ? 'text-ink-green-7' : 'text-ink-gray-6'"
        >
          {{ day.label }}<span v-if="day.isToday"> · {{ __('hoy') }}</span>
        </span>
        <button
          class="press px-1 text-[13px] text-ink-gray-5"
          :aria-label="__('Nueva publicación este día')"
      ...
      <button
        ...
        :class="[
          pillarChip(p.post_kind),
          p.status === 'Cancelado' ? 'line-through opacity-60' : '',
        ]"
      ...
      >
        <span class="mr-0.5 inline-flex items-center gap-px align-middle">
          ...
          <span
            v-if="p.recycled_from"
            class="opacity-60"
            :title="__('Reciclado')"
            >↺</span
          >
          <span
            v-for="cb in channelBadges(p.channels)"
            :key="cb.channel"
            class="inline-flex items-center"
            :title="cb.channel + ' · ' + cb.status"
          >
            <span>{{ cb.emoji }}</span>
            <span
              class="ml-px inline-block size-1.5 shrink-0 rounded-full"
              :class="cb.dot"
            />
          </span> </span
        ><span v-if="hm(p)" class="mr-1 tabular-nums text-[11px] opacity-75">{{
          hm(p)
        }}</span
        >{{ p.title || p.name }}
      </button>
  ...
  <div v-else class="p-4">
    <!-- weekday header -->
    <div
      class="grid grid-cols-7 gap-1 text-center text-[10px] font-bold uppercase tracking-wide text-ink-gray-4"
    >
    ...
    <div class="mt-1 grid grid-cols-7 gap-1">
      <div
        ...
        :class="[
          isBlackout(day)
            ...
            : day.isToday
              ? 'border-green-400 dark:border-green-500 bg-surface-base'
              : day.inMonth
                ? 'border-outline-gray-2 bg-surface-base hover:border-outline-gray-3'
                : 'border-transparent bg-surface-gray-1/50',
          dragOver === day.key
        ...
        :data-testid="
          isBlackout(day) ? `cal-blackout-${day.key}` : `cal-day-${day.key}`
        "
        @click="emit('open-new', day)"
      ...
      >
        ...
        <div
          ...
          :class="
            day.isToday ? 'font-bold text-ink-green-7' : 'text-ink-gray-6'
          "
        >
          <span
            v-if="isBlackout(day)"
            class="mr-0.5"
            :title="__('Día bloqueado')"
            >🚫</span
          >{{ day.n }}
        </div>
        <div class="flex flex-col gap-0.5">
          ...
          >
            <span class="mr-0.5 inline-flex items-center gap-px align-middle">
              <span v-if="p.evergreen" :title="__('Evergreen (rotable)')"
                >♻</span
              >
              <span
                v-if="p.recycled_from"
            ...
            ><span v-if="hm(p)" class="mr-0.5 tabular-nums opacity-70">{{
              hm(p)
            }}</span
            >{{ p.title || p.name }}
          </button>
    ...
    <div class="mt-3 flex flex-wrap items-center gap-1.5 text-[10.5px]">
      ...
      <span
        ...
        :class="pl.chip"
        >{{ pl.emoji }} {{ pl.kind }}</span
      >
    </div>
    <!-- channel status-dot legend -->
    <div
      class="mt-1.5 flex flex-wrap items-center gap-2 text-[10.5px] text-ink-gray-6"
    >
      <span class="font-semibold text-ink-gray-5">{{ __('Canal') }}:</span>
      <span
        v-for="sd in DOT_LEGEND"
        :key="sd.status"
        class="inline-flex items-center gap-1"
      >
        <span class="inline-block size-1.5 rounded-full" :class="sd.dot" />{{
          sd.label
        }}
      </span>
    </div>
    ...
    <div v-if="drafts.length" class="mt-4">
      <div
        class="mb-1 text-[10px] font-bold uppercase tracking-wide text-ink-gray-4"
      >
        {{ __('Borradores sin programar') }}
      </div>
...
function cellPosts(day) {
  ...
  if (
    ...
    expandedDays.value.has(day.key)
  )
    return list
...
const blackoutsRes = createResource({
  ...
  makeParams: () => ({
    start: props.days[0]?.key,
    end: props.days[props.days.length - 1]?.key,
  }),
  auto: true,
})
