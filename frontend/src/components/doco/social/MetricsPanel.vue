...
<template>
  <div class="p-4">
    ...
    <span
      class="mb-2 block text-[10px] font-bold uppercase tracking-wide text-ink-gray-4"
      >{{ __('Mejores horarios') }}</span
    >
    <div
      class="mb-5 rounded-[12px] border border-outline-gray-2 bg-surface-base p-3"
    >
    ...
    <div class="mb-2 flex items-center justify-between">
      <span
        ...
        >{{ __('Por sucursal') }}</span
      >
      <button
        class="rounded-md border border-outline-gray-2 px-2 py-1 text-[11px] font-semibold text-ink-gray-7 hover:bg-surface-gray-2"
    ...
    <div
      class="mb-5 overflow-x-auto rounded-[12px] border border-outline-gray-2 bg-surface-base"
    >
      <table class="w-full min-w-[560px] text-[12.5px]">
        <thead
          class="bg-surface-gray-2 text-[10px] font-bold uppercase tracking-wide text-ink-gray-5"
        >
          <tr>
            ...
            <th class="px-3 py-2 text-right text-ink-green-7">
              {{ __('Leads WA') }}
            </th>
          </tr>
        </thead>
        <tbody>
          ...
          >
            ...
            <td class="px-3 py-2 text-right tabular-nums">
              {{ r.link_clicks.toLocaleString() }}
            </td>
            <td
              class="px-3 py-2 text-right font-bold tabular-nums text-ink-green-7"
            >
              {{ r.leads }}
            </td>
          </tr>
          <tr
            v-if="(lb.data || []).length > 1"
            class="border-t-2 border-outline-gray-2 bg-surface-gray-1 font-bold"
          >
            ...
            <td class="px-3 py-2 text-right tabular-nums">
              {{ lbTotals.reach.toLocaleString() }}
            </td>
            <td class="px-3 py-2 text-right tabular-nums">
              {{ lbTotals.engagement.toLocaleString() }}
            </td>
            <td class="px-3 py-2 text-right tabular-nums">
              {{ lbTotals.link_clicks.toLocaleString() }}
            </td>
            <td class="px-3 py-2 text-right tabular-nums text-ink-green-7">
              {{ lbTotals.leads }}
            </td>
          </tr>
          <tr v-if="!(lb.data || []).length">
            <td colspan="6" class="px-3 py-6 text-center text-ink-gray-4">
              {{
                lb.loading
                  ? __('Cargando…')
                  : __('Sin datos por sucursal todavía.')
              }}
            </td>
          </tr>
        </tbody>
    ...
    <span
      class="mb-2 block text-[10px] font-bold uppercase tracking-wide text-ink-gray-4"
      >{{ __('Por publicación') }}</span
    >
    <div
      class="overflow-x-auto rounded-[12px] border border-outline-gray-2 bg-surface-base"
    >
      <table class="w-full min-w-[520px] text-[12.5px]">
        ...
        <tbody>
          ...
          >
            <td class="px-3 py-2">
              ...
              <div class="text-2xs text-ink-gray-4">
                {{ row.status }} · {{ (row.scheduled_time || '').slice(0, 10) }}
              </div>
            </td>
            <td class="px-3 py-2 text-right tabular-nums">
          ...
          <tr v-if="!(dash.data || []).length">
            <td colspan="5" class="px-3 py-8 text-center text-ink-gray-4">
              {{
                dash.loading
                  ? __('Cargando…')
                  : __(
                      'Sin datos todavía (las métricas se refrescan a diario).',
                    )
              }}
            </td>
          </tr>
...
function exportLeaderboard() {
  ...
  if (!rows.length) {
    toast.error(__('Sin datos para exportar'))
    return
  }
  const cols = [
    'shop_name',
