  ...
  >
    ...
    <div class="flex items-center gap-2.5 px-3 pt-3">
      ...
      <div class="min-w-0 flex-1">
        <div
          class="truncate text-[14px] font-semibold leading-tight text-ink-gray-9"
        >
          {{ pageName || __('Página de Facebook') }}
        </div>
        <div class="flex items-center gap-1 text-[11.5px] text-ink-gray-5">
          <span class="truncate">{{ timeLabel || '' }}</span>
          <span v-if="timeLabel">·</span>
          <svg viewBox="0 0 16 16" class="h-3 w-3 flex-none fill-current">
            <path
              d="M8 1a7 7 0 100 14A7 7 0 008 1zM2.5 8a5.5 5.5 0 011-3.2c.2.5.6.9 1.1 1 .5.2.5.7.2 1.1-.3.4-.1.9.3 1.1.5.2.5.8 0 1.1-.4.2-.5.8-.2 1.2.3.4.1.9-.3 1.1-.2.1-.4.3-.4.6A5.5 5.5 0 012.5 8zm6 5.4V12c0-.6-.4-1-1-1-.8 0-1.5-.7-1.5-1.5S6.7 8 7.5 8H9c.6 0 1-.4 1-1 0-.4.3-.8.7-.9A5.5 5.5 0 018.5 13.4z"
            />
          </svg>
        </div>
      </div>
    ...
    <div
      v-if="message"
      class="whitespace-pre-wrap [overflow-wrap:anywhere] px-3 py-2 text-[13.5px] leading-snug text-ink-gray-9"
    >
      {{ message }}
    </div>
    ...
    <div
      v-if="cta"
      class="flex items-center justify-between gap-2 border-t border-outline-gray-1 bg-surface-gray-1 px-3 py-2"
    >
      <span class="truncate text-[12.5px] text-ink-gray-6">{{
        cta.label
      }}</span>
      <span
        class="flex-none rounded-md bg-surface-gray-3 px-3 py-1 text-[12px] font-semibold text-ink-gray-8"
        >{{ cta.button || __('Más info') }}</span
      >
    </div>
    ...
    <div
      v-if="hasCounts"
      class="flex items-center gap-3 border-t border-outline-gray-1 px-3 py-1.5 text-[12px] text-ink-gray-5"
    >
    ...
    <div
      v-if="showActions"
      class="grid grid-cols-3 border-t border-outline-gray-1 text-[13px] font-semibold text-ink-gray-6"
    >
      <span class="flex items-center justify-center gap-1.5 py-2"
        >👍 {{ __('Me gusta') }}</span
      >
      <span
        class="flex items-center justify-center gap-1.5 border-x border-outline-gray-1 py-2"
        >💬 {{ __('Comentar') }}</span
      >
      <span class="flex items-center justify-center gap-1.5 py-2"
        >↗ {{ __('Compartir') }}</span
...
const hasCounts = computed(
  () =>
    props.reactions != null || props.comments != null || props.shares != null,
)
</script>
