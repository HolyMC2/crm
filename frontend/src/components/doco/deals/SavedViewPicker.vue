...
<template>
  <div class="relative inline-block">
    ...
    >
      {{ __('Vistas')
      }}<span v-if="selectedView"> · {{ selectedView.label }}</span> ⌄
    </button>
    ...
    <template v-if="open">
      ...
      >
        ...
        >
          <span
            class="text-[10.5px] font-semibold uppercase tracking-[.07em] text-ink-gray-4"
            >{{ __('Vistas guardadas') }}</span
          >
          <button
            v-if="selectedView"
            class="text-[11.5px] text-ink-gray-5 hover:text-ink-gray-8"
        ...
        <p
          v-if="views.loading && !views.data"
          class="px-3 py-3 text-[12px] text-ink-gray-4"
        >
          {{ __('Cargando vistas…') }}
        </p>
        <div v-else-if="views.error" class="px-3 py-3">
          <p role="alert" class="text-[12px] text-ink-red-8">
            {{ __('No se pudieron cargar las vistas guardadas.') }}
          </p>
          <button
            class="mt-1 text-[12px] font-semibold underline"
            @click="views.reload()"
          >
            {{ __('Reintentar') }}
          </button>
        ...
        <template v-for="group in groups" :key="group.key">
          <div v-if="group.rows.length">
            <div
              class="border-b border-outline-gray-1 bg-surface-gray-1 px-3 py-1 text-[10.5px] font-semibold uppercase tracking-[.07em] text-ink-gray-4"
            >
              {{ group.label }}
            </div>
            <div
              ...
              :class="
                String(v.name) === String(selected) ? 'bg-surface-green-2' : ''
              "
            >
              <button
                class="min-w-0 flex-1 truncate text-left text-[13px] text-ink-gray-8"
                @click="apply(v)"
              >
                {{ v.label }}
                <span
                  v-if="v.public"
                  class="ml-1 rounded bg-surface-gray-3 px-1 py-px text-[10px] font-semibold text-ink-gray-6"
                  >{{ __('Pública') }}</span
                >
                <span
                  v-if="v.is_default"
                  class="ml-1 text-[10px] text-ink-gray-5"
                  >★ {{ __('Predeterminada') }}</span
                >
              </button>
              <button
        ...
        <p
          v-if="!views.loading && !anyView && !legacyViews.length"
          class="px-3 py-3 text-[12px] text-ink-gray-4"
        >
          {{
            __(
              'Aún no hay vistas. Guarda tus filtros, colas y columnas actuales para reutilizarlas o compartirlas.',
            )
          }}
        </p>
        ...
        <div class="flex flex-col gap-1 px-3 py-2">
          ...
          <button
            v-if="selectedView"
            class="rounded-lg border border-outline-gray-2 px-3 py-1.5 text-[12px] font-medium text-ink-gray-7 disabled:opacity-60"
            :disabled="busy"
...
const selectedView = computed(
  () =>
    rows.value.find((v) => String(v.name) === String(props.selected)) || null,
)
const groups = computed(() => [
  {
    key: 'pinned',
    label: __('Fijadas'),
    rows: rows.value.filter((v) => v.pinned),
  },
  {
    key: 'mine',
    label: __('Mis vistas'),
    rows: rows.value.filter((v) => !v.pinned && !v.public),
  },
  {
    key: 'public',
    label: __('Del equipo'),
...
function saveAsNew() {
  inputDialog({
    ...
    onConfirm: (label) =>
      run(async () => {
        const doc = await call(`${SETTINGS}.create`, {
          view: viewPayload(props.context, { label }),
        })
        await views.reload()
...
function updateSelected() {
  ...
  run(async () => {
    await call(`${SETTINGS}.update`, {
      view: viewPayload(props.context, {
        label: current.label,
        icon: current.icon,
        name: current.name,
      }),
...
function remove(v) {
  confirmDialog({
    title: __('Eliminar vista'),
    message: __(
      '¿Eliminar la vista «{0}»? Si es pública, dejará de estar disponible para el equipo.',
      [v.label],
    ),
    ...
    onConfirm: () =>
      run(async () => {
        ...
        if (String(v.name) === String(props.selected))
          emit('update:selected', '')
        await views.reload()
...
async function run(work) {
  ...
  try {
    ...
    toast.error(
      e?.messages?.[0] ||
        __(
          'No se pudo guardar la vista. Revisa tus permisos sobre las vistas del equipo.',
        ),
    )
  } finally {
    busy.value = false
