    ...
    >
      ...
      >
        ...
        <div class="flex-none px-4 pb-1 pt-2.5">
          <div
            class="mx-auto mb-2.5 h-1 w-9 rounded-full bg-surface-gray-4"
            aria-hidden="true"
          />
        ...
        <div class="scb min-h-0 flex-1 overflow-y-auto px-4">
          <div
            v-for="g in groups"
            :key="g.key"
            class="border-b border-outline-gray-1 py-2 last:border-b-0"
          >
            <div
              class="pb-1 text-[10.5px] font-bold uppercase tracking-[.07em] text-ink-gray-4"
            >
              {{ g.label }}
            </div>
            <label
            ...
            <div
              v-if="!g.options.length"
              class="py-2 text-[12px] text-ink-gray-4"
            >
              {{ __('Sin opciones') }}
            </div>
          </div>
        ...
        <div
          class="flex flex-none items-center gap-2 border-t border-outline-gray-1 px-4 py-2.5"
        >
          <button
...
const resultLabel = computed(() =>
  props.count == null
    ? __('Listo')
    : __('Ver {0} resultados', [String(props.count)]),
)
