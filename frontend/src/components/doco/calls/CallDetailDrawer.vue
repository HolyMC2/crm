...
<template>
  ...
  >
    ...
    <div
      class="scb relative z-10 flex h-full w-[460px] flex-col overflow-y-auto bg-surface-base shadow-2xl"
    >
      <!-- header -->
      <div
      ...
      >
        <div class="min-w-0">
          <div class="text-[14px] font-bold text-ink-gray-9">
            {{ __('Detalle de llamada') }}
          </div>
          <div class="text-[12px] text-ink-gray-5">
            {{ dirLabel }} · {{ fmtDur(d.duration) }}
          </div>
        </div>
        <button
          class="text-[18px] text-ink-gray-5 hover:text-ink-gray-9"
      ...
      <div class="flex-1 px-5 py-4">
        ...
        <div class="mb-4 flex items-center gap-3">
          <span
            ...
            :class="outcomeChip(d.status)"
            >{{ d.status || '—' }}</span
          >
          <div
            v-if="d.call_quality_score != null"
            ...
            <div class="relative h-10 w-10">
              ...
              >
                ...
                <circle
                  ...
                  :stroke-dashoffset="qualityOffset"
                />
              </svg>
              <div
                class="absolute inset-0 flex items-center justify-center text-[11px] font-bold text-ink-gray-9"
              >
                {{ d.call_quality_score }}
              </div>
            </div>
        ...
        <div v-if="d.recording_url_path" class="mb-4">
          <div
            class="mb-1.5 text-[11px] font-semibold uppercase tracking-[.07em] text-ink-gray-4"
          >
            {{ __('Grabación') }}
          </div>
        ...
        <div
          v-if="d.ai_summary"
          class="mb-4 rounded-[10px] border-l-[3px] bg-surface-green-2 p-3"
          style="border-color: var(--outline-green-4)"
        >
          <div
            class="mb-1 text-[11px] font-semibold uppercase tracking-[.07em] text-ink-green-7"
          >
            {{ __('Resumen IA') }}
          </div>
          <div class="text-[12.5px] leading-relaxed text-ink-gray-8">
            {{ d.ai_summary }}
          </div>
        </div>
        <button
        ...
        >
          {{
            d.transcript
              ? __('Generar resumen IA')
              : __('Sin transcripción aún')
          }}
        </button>
        ...
        <div v-if="d.transcript" class="mb-4">
          ...
          <div
            class="whitespace-pre-wrap rounded-[10px] bg-surface-gray-1 p-3 text-[12px] leading-relaxed text-ink-gray-7"
          >
            {{ d.transcript }}
          </div>
        </div>
        ...
        <!-- detail rows -->
        <div
          v-if="d.tone || d.talk_ratio || d.next_step_set"
          class="mb-4 rounded-[10px] border border-outline-gray-2"
        >
        ...
        <div class="mt-2 flex gap-2">
          <button
            class="rounded-lg border border-outline-gray-2 px-3 py-1.5 text-[12px] font-medium text-ink-gray-7"
            :disabled="saving"
            @click="saveNote"
          >
            {{ __('Guardar nota') }}
...
const detail = createResource({
  url: 'doco_marketing.api.calls.get_call_detail',
})
const note = ref('')
watch(
  () => props.name,
...
const dirLabel = computed(() =>
  d.value.type === 'Outgoing' ? __('Saliente') : __('Entrante'),
)
const qualityOffset = computed(
  () => (100.5 * (100 - (Number(d.value.call_quality_score) || 0))) / 100,
)
...
function outcomeChip(status) {
  ...
  if (['No Answer', 'Missed', 'Busy', 'Failed', 'Canceled'].includes(status))
    return 'text-ink-red-8 bg-surface-red-1'
...
async function saveNote() {
  ...
  try {
    ...
    await frappeCall('doco_marketing.api.calls.save_call_note', {
      name: props.name,
      text: note.value,
    })
...
async function transcribe() {
  ...
  try {
    await frappeCall('doco_marketing.api.calls.enqueue_transcription', {
      name: props.name,
    })
...
const Row = (props) =>
  h(
    ...
    {
      class:
        'flex justify-between border-b border-outline-gray-1 px-3 py-2 text-[12.5px] last:border-0',
    },
    [
