...
<template>
  <div>
    ...
    <div
      v-if="frozen"
      class="mb-3 max-w-[520px] rounded-[10px] border border-outline-amber-2 bg-surface-amber-1 px-3 py-2 text-[12px] text-ink-amber-3"
    >
      {{ frozenHint }}
    </div>
        ...
        <div class="rounded-[11px] border border-outline-gray-2 p-3">
          ...
          <div class="mb-2 flex items-center gap-2">
            ...
            <span
              ...
              class="flex-none rounded-full bg-surface-amber-1 px-2 py-[2px] text-[10.5px] font-semibold text-ink-amber-3"
            >
              ⏳ {{ s.wait_hours }}h
            </span>
            ...
            <button
              v-if="!frozen"
              class="flex-none text-ink-gray-4 hover:text-ink-red-4"
              @click="removeStep(i)"
          ...
          <div v-if="isPreviewOpen(s) && previewTpl(s)" class="mt-2">
            <div class="rounded-[10px] bg-surface-green-2 px-3 py-2">
              <div class="whitespace-pre-wrap text-[12px] leading-relaxed text-ink-gray-8">
                <template v-for="(part, k) in tplParts(previewTpl(s).template)" :key="k">
                  <span
                    v-if="k % 2 === 1"
                    class="mx-[1px] rounded bg-surface-amber-1 px-1 font-mono text-[11px] font-semibold text-ink-amber-3"
                  >{{ part }}</span>
            ...
            <div class="mt-1 flex items-center gap-2 text-[10.5px] text-ink-gray-4">
              ...
              <span
                ...
                class="rounded bg-surface-amber-1 px-1.5 py-[1px] font-semibold text-ink-amber-3"
              >
                {{ previewTpl(s).status }}
              </span>
            </div>
