    ...
    <template #body>
      <div class="px-4 py-4 sm:px-6">
        <div class="mb-3 flex items-center justify-between">
          <h3 class="text-[15px] font-bold text-ink-gray-9">
            {{ __('Bitácora') }}
          </h3>
          <button
        ...
        <div class="mb-3 flex flex-wrap gap-2">
          ...
          >
            <span
              ...
              :style="`background:${chColor(ch)}`"
            />
            {{ ch }} · {{ n }}
          </span>
          <span
            v-if="!Object.keys(counts).length && !ledger.loading"
            class="text-[12px] text-ink-gray-5"
          >
        ...
        <div
          ...
          class="mb-3 rounded-lg border border-outline-gray-1 bg-surface-gray-1 px-3 py-2"
        >
          <div class="mb-1.5 flex items-center justify-between">
            <div
              class="text-[10px] font-bold uppercase tracking-wide text-ink-gray-5"
            >
              {{ __('Consentimiento') }}
            </div>
            <a
          ...
          >
            ⚠
            {{
              __('{0} registro(s) no pasan la verificación de integridad', [
                consentTampered,
              ])
            }}
          </div>
          <div class="flex flex-wrap gap-1.5">
            ...
            >
              ...
              <span class="font-normal opacity-80"
                >· {{ classLabel(r.consent_class) }} · {{ fmtDay(r.ts) }}</span
              >
        ...
        >
          ...
          <div class="flex flex-wrap gap-x-4 gap-y-1 text-[12px]">
            <span
              v-for="(id, i) in identities"
              :key="i"
              class="inline-flex items-center gap-1.5"
            >
              <span
                class="font-semibold"
                :style="`color:${chColor(id.channel)}`"
                >{{ id.channel }}</span
              >
        ...
        <div class="scb max-h-[55vh] overflow-y-auto">
          ...
          >
            <span
              ...
              :style="`background:${chColor(m.channel)}`"
            />
            <div class="min-w-0 flex-1">
              <div
                class="flex items-center gap-1.5 text-[10.5px] text-ink-gray-5"
              >
                <span
                  class="font-semibold"
                  :style="`color:${chColor(m.channel)}`"
                  >{{ m.channel }}</span
                >
                <span
                  >·
                  {{
                    m.direction === 'out' ? __('enviado') : __('recibido')
                  }}</span
                >
                <span>· {{ fmt(m.ts) }}</span>
              </div>
              <div class="truncate text-[13px] text-ink-gray-8">
                {{
                  m.content ||
