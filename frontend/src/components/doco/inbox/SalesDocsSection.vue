...
<template>
  ...
  >
    ...
    <div
      v-if="salesSummary.loading && !salesSummary.data"
      class="py-2 text-[12px] text-ink-gray-5"
    >
      {{ __('Cargando…') }}
    </div>
    <div
      v-else-if="salesSummary.error && !salesSummary.data"
      class="py-2 text-[12px] text-ink-red-6"
    >
    ...
    <template v-else-if="summary">
      ...
      <div v-if="rollup && rollup.invoiced" class="mb-2.5">
        <div class="grid grid-cols-3 gap-1.5 text-center">
          <div class="rounded-lg bg-surface-gray-2 px-1.5 py-1.5">
            <div
              class="text-[9.5px] font-bold uppercase tracking-wide text-ink-gray-5"
            >
              {{ __('Facturado') }}
            </div>
            <div class="text-[12.5px] font-bold tabular-nums text-ink-gray-9">
              {{ money(rollup.invoiced) }}
            </div>
          </div>
          <div class="rounded-lg bg-surface-green-2 px-1.5 py-1.5">
            <div
              class="text-[9.5px] font-bold uppercase tracking-wide text-ink-green-7"
            >
              {{ __('Pagado') }}
            </div>
            <div class="text-[12.5px] font-bold tabular-nums text-ink-green-7">
              {{ money(rollup.paid) }}
            </div>
          </div>
          <div
            ...
            :class="
              rollup.outstanding > 0 ? 'bg-surface-red-1' : 'bg-surface-gray-2'
            "
          >
            <div
              class="text-[9.5px] font-bold uppercase tracking-wide"
              :class="
                rollup.outstanding > 0 ? 'text-ink-red-8' : 'text-ink-gray-5'
              "
            >
              {{ __('Saldo') }}
            </div>
            <div
              class="text-[12.5px] font-bold tabular-nums"
              :class="
                rollup.outstanding > 0 ? 'text-ink-red-8' : 'text-ink-gray-7'
              "
            >
              {{ money(rollup.outstanding) }}
            </div>
          </div>
        </div>
        <div
          class="mt-1.5 h-1 overflow-hidden rounded-full bg-surface-gray-3"
          :title="__('Pagado vs facturado')"
        >
          <div
      ...
      <div v-if="!hasAny" class="py-1 text-[12px] text-ink-gray-5">
        ...
        <span class="text-ink-gray-4">{{
          __('Abre Artículos, agrega los productos y pulsa Cotizar.')
        }}</span>
      </div>
      ...
      <div v-else class="flex flex-col gap-2.5">
        <div v-for="g in groups" :key="g.label">
          <div
            class="mb-1 text-[10px] font-bold uppercase tracking-wide text-ink-gray-5"
          >
            ...
            >
              ...
              >
                <div class="flex items-center gap-1.5">
                  ...
                  <span
                    v-if="d.doctype === 'POS Invoice'"
                    class="rounded bg-surface-gray-2 px-1 py-px text-[9.5px] text-ink-gray-5"
                    >POS</span
                  >
                </div>
                <div class="mt-0.5 flex items-center gap-1.5">
                  ...
                  <span class="text-[10px] text-ink-gray-5">{{
                    d.transaction_date || d.posting_date
                  }}</span>
                </div>
              </button>
              <button
                ...
                :title="
                  d.docstatus === 0
                    ? __('Editar / enviar cotización')
                    : __('Enviar / convertir cotización')
                "
              ...
              <div class="flex flex-none flex-col items-end gap-0.5">
                <span class="font-semibold tabular-nums text-ink-gray-8">{{
                  money(d.grand_total, d.currency)
                }}</span>
                <span
                  v-if="
                    d.doctype &&
                    ...
                    Number(d.outstanding_amount) > 0
                  "
                  class="text-[10px] font-semibold tabular-nums text-ink-red-7"
                >
        ...
        <div v-if="payments.length">
          ...
          >
            <div
              v-for="p in payments"
              :key="p.name"
              class="flex items-center justify-between gap-2 px-2 py-1.5 text-[11.5px]"
            >
              <div class="min-w-0">
                ...
                <div class="mt-0.5 text-[10px] text-ink-gray-5">
                  {{ p.posting_date
                  }}<template v-if="p.mode_of_payment">
                    · {{ p.mode_of_payment }}</template
                  >
                  <template v-if="p.payment_type !== 'Receive'">
                    · {{ __('egreso') }}</template
                  >
                </div>
              </div>
              <span
                class="flex-none font-semibold tabular-nums text-ink-green-7"
                >{{ money(p.allocated_amount) }}</span
              >
            </div>
          </div>
        </div>
    ...
    <Dialog v-model="viewerOpen" :options="{ size: '4xl' }">
      <template #body>
        ...
        >
          <h3 class="text-[14px] font-bold text-ink-gray-9">
            {{ viewer.name }}
          </h3>
  ...
  [
    {
      ...
      doctype: 'Quotation',
      docs: summary.value?.quotations || [],
    },
    {
      label: __('Órdenes de venta'),
      doctype: 'Sales Order',
      docs: summary.value?.sales_orders || [],
    },
    {
      label: __('Facturas'),
      doctype: 'Sales Invoice',
      docs: summary.value?.invoices || [],
    },
...
function onErp(payload) {
  if (payload?.channel === 'erp' && payload?.deal === props.deal)
    reloadSalesSummary()
}
...
const STATUS_ES = {
  ...
  Closed: 'Cerrada',
  'To Deliver and Bill': 'Por entregar y facturar',
