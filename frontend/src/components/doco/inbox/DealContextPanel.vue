...
async function macroListo() {
  ...
  if (!ready) {
    toast.error(
      __(
        'Configura una etapa visible de reparación lista antes de usar esta acción.',
      ),
    )
    return
  }
  try {
    ...
    toast.error(
      e?.messages?.[0] || __('No se pudo actualizar la etapa de reparación.'),
    )
    return
  }
  openTemplate()
  toast.success(
    __('Estado actualizado: {0}. Revisa y envía la plantilla.', [ready.name]),
  )
}
