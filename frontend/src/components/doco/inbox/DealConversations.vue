<template>
  ...
  >
    ...
    <component
      ...
      v-bind="
        item.name
          ? { to: conversationRoute(item.name) }
          : { type: 'button', disabled: opening === itemKey(item) }
      "
  ...
  <section
    ...
    aria-label="Conversaciones vinculadas"
  >
    <div>
      <h2 class="text-base font-semibold">
        {{ __('Conversaciones vinculadas') }}
      </h2>
      <p class="mt-1 text-sm text-ink-gray-5">
        {{
          __(
            'Abre una conversación para ver su historial y responder desde su cuenta.',
          )
        }}
      </p>
    </div>
        ...
        >
          <span class="min-w-0">
            <span class="block truncate text-sm font-medium text-ink-gray-9">{{
              item.display_name || item.peer_id
            }}</span>
            <span class="block text-xs text-ink-gray-5">
              {{ item.provider }} · {{ __('Cuenta') }} {{ item.account_id }}
              <template v-if="item.last_message_at">
                · {{ age(item.last_message_at) }}</template
              >
            </span>
            <span
              v-if="item.preview"
              class="mt-1 block truncate text-sm text-ink-gray-7"
              >{{ item.preview }}</span
            >
          </span>
          <span class="shrink-0 text-sm text-ink-gray-5">
            {{
              item.name
                ? __(item.control_state)
                : opening === itemKey(item)
                  ? __('Abriendo…')
                  : __('Abrir historial')
            }}
            →
    ...
    <p v-else-if="!loading && !error" class="text-sm text-ink-gray-5">
      {{
        __(
          'No hay conversaciones vinculadas disponibles para este registro. Puedes buscar en las cuentas a las que tienes acceso.',
        )
      }}
    </p>
    <button
      ...
      @click="load"
    >
      {{ error ? __('Reintentar') : __('Cargar más') }}
    </button>
    <RouterLink
      :to="{ name: 'Inbox', query: { workspace: 'conversations' } }"
      class="self-start text-sm font-medium text-ink-green-7 underline"
      >{{ __('Abrir conversaciones') }}</RouterLink
    >
  </section>
</template>
...
const itemKey = (item) =>
  item.name || `${item.provider}:${item.account_id}:${item.peer_id}`
const conversationRoute = (name) => ({
  name: 'Inbox',
  query: { workspace: 'conversations', conversation: name },
})
...
async function load() {
  ...
  try {
    const result = await call(
      'crm.api.conversation_threads.list_for_reference',
      { doctype: props.doctype, name: props.name, cursor: cursor.value },
    )
    items.value.push(...result.items)
    cursor.value = result.next_cursor
  } catch {
    error.value = __(
      'No se pudieron cargar las conversaciones. Reintenta o revisa tu acceso a la cuenta.',
    )
  } finally {
    loading.value = false
  }
...
async function open(item) {
  ...
  try {
    ...
    error.value = __(
      'No se pudo abrir la conversación. Reintenta o revisa tu acceso a la cuenta.',
    )
  } finally {
    opening.value = ''
  }
