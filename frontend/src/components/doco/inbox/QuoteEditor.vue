  ...
  <div class="rounded-lg border border-outline-gray-2 bg-surface-gray-1 p-2">
    <div class="mb-1.5 flex items-center justify-between">
      <span class="text-[11px] font-bold text-ink-gray-8">
        ...
        <span
          ...
          class="ml-1 rounded bg-surface-gray-3 px-1 py-px text-[9.5px] font-semibold text-ink-gray-6"
          >{{ __('Borrador') }}</span
        >
      </span>
      <button
        class="text-ink-gray-5 hover:text-ink-gray-9"
        :aria-label="__('Cerrar editor')"
        @click="$emit('close')"
      >
        ✕
      </button>
    ...
    <template v-else-if="detail">
        ...
        >
          <div class="flex items-center justify-between gap-2">
            <span
              ...
              :title="l.item_code"
              >{{ l.item_name || l.item_code }}</span
            >
            <span
              class="flex-none font-semibold tabular-nums text-ink-gray-9"
              >{{ money(l.amount) }}</span
            >
          </div>
          <div v-if="editable" class="mt-1 flex items-center gap-2">
            ...
            <label class="flex items-center gap-1 text-[10px] text-ink-gray-5">
              ...
              <input
                ...
                :disabled="busy"
                @change="
                  patchLine(l, { discount_percentage: $event.target.value })
                "
              />%
            </label>
          ...
          <div v-else class="mt-0.5 text-[10px] text-ink-gray-5">
            {{ l.qty }} × {{ money(l.rate)
            }}<template v-if="l.discount_percentage">
              · −{{ l.discount_percentage }}%</template
            >
          </div>
        </div>
      </div>
      ...
      <div class="mt-1.5 flex items-center justify-between text-[12px]">
        <span
          class="font-bold uppercase tracking-wide text-ink-gray-5 text-[10px]"
          >{{ __('Total') }}</span
        >
        <span class="font-bold tabular-nums text-ink-gray-9">{{
          money(detail.total)
        }}</span>
      </div>
      ...
      <div class="mt-2 flex flex-wrap gap-1.5">
        <button
          ...
          :title="
            __('Enviar la cotización en PDF a la conversación de WhatsApp')
          "
          @click="sendWhatsapp"
        >
...
async function patchLine(l, patch) {
  const out = await run(() =>
    updateQuotationLine(props.deal, props.quotation, l.name, patch),
  )
  if (out) detail.value = out
}
async function dropLine(l) {
  const out = await run(() =>
    removeQuotationLine(props.deal, props.quotation, l.name),
  )
  if (out?.deleted) {
...
async function submit() {
  const out = await run(
    () => submitQuotation(props.deal, props.quotation),
    __('Cotización emitida.'),
  )
  if (out) detail.value = out
}
