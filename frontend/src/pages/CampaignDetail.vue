...
<template>
  <div
    class="scb flex min-h-0 w-full flex-1 flex-col overflow-y-auto bg-surface-base"
  >
    <!-- header -->
    <div
    ...
    >
      <div class="flex min-w-0 items-center gap-2">
        ...
        <input
          v-model="form.title"
          class="min-w-0 border-0 bg-transparent text-[15px] font-bold text-ink-gray-9 focus:outline-none focus:ring-0"
          :placeholder="__('Nombre de la campaña')"
        />
        <span
          class="rounded-md px-2 py-[3px] text-[11px] font-semibold"
          :class="statusChip(form.status)"
          >{{ form.status }}</span
        >
        <span
          v-if="isCadence"
          class="flex-none rounded-md bg-surface-violet-2 px-2 py-[3px] text-[11px] font-semibold text-ink-violet-8"
          >{{ __('Cadencia 1:1') }}</span
        >
      </div>
      <div class="flex items-center gap-2">
        ...
        <button
          v-else-if="form.status === 'Paused'"
          class="rounded-lg border border-outline-green-4 bg-surface-green-2 px-3 py-1.5 text-[12.5px] font-semibold text-ink-green-8"
    ...
    <!-- settings -->
    <div
      class="flex flex-none flex-wrap items-end gap-4 border-b border-outline-gray-1 px-5 py-3"
    >
      <Field :label="__('Tipo')">
        <select v-model="form.type" class="dm-input">
      ...
      <Field :label="__('Flujo')">
        <label
          ...
          :title="
            cadenceLocked
              ? __('No se puede cambiar: la campaña ya tiene inscritos.')
              : ''
          "
        >
          <input
      ...
      <template v-if="!isCadence">
        ...
        <Field
          v-if="form.enrollment_trigger === 'stage_entered'"
          :label="__('Etapa')"
        >
          <select v-model="form.trigger_value" class="dm-input">
            <option value="">—</option>
            <option
              v-for="s in dealStatuses.data || []"
              :key="s.name"
              :value="s.name"
            >
              {{ s.name }}
        ...
        <Field
          v-else-if="form.enrollment_trigger === 'score_threshold'"
          :label="__('Score ≥')"
        >
          <input
        ...
        <Field :label="__('Audiencia')">
          <select v-model="form.audience" class="dm-input">
            <option value="">—</option>
            <option
              v-for="a in audiences.data || []"
              :key="a.name"
              :value="a.name"
            >
              {{ a.title || a.name }}
        ...
        <button
          class="rounded-lg border border-outline-gray-2 px-3 py-1.5 text-[12px] font-medium text-ink-gray-7 disabled:opacity-50"
          :disabled="!form.audience || enrolling"
          @click="doEnroll"
        >
          {{
      ...
      <div
        v-else
        class="flex h-[29px] items-center text-[11.5px] text-ink-gray-5"
      >
    ...
    <div class="flex min-h-0 flex-1">
      ...
      <div
        class="scb min-h-0 flex-1 overflow-y-auto border-r border-outline-gray-1 p-5"
      >
        <StepCardList
          v-model="form.steps"
          kind="campaign"
          ...
          :frozen-hint="
            __(
              'Los pasos están congelados mientras la campaña está Activa — pausa para editarlos (las inscripciones en curso siguen la posición de cada paso).',
            )
          "
        />
      </div>
        ...
        >
          ...
          <div class="min-w-0 flex-1">
            ...
            <div class="text-[11px] text-ink-gray-4">
              {{ __('Paso') }} {{ (e.current_step ?? 0) + 1 }} ·
              {{ e.last_sent_at ? timeAgo(e.last_sent_at) : '—' }}
            </div>
          </div>
          <span
            class="flex-none rounded px-2 py-[2px] text-[10.5px] font-semibold"
            :class="enrStatusChip(e.status)"
            >{{ e.status }}</span
          >
        </div>
      </div>
...
const campaign = createResource({
  url: 'doco_marketing.api.campaigns.get_campaign',
})
const enroll = createResource({
  url: 'doco_marketing.api.campaigns.get_enrollments',
})
const audiences = createListResource({
  doctype: 'Marketing Audience',
  fields: ['name', 'title'],
  pageLength: 100,
  auto: true,
})
...
const cadenceLocked = computed(() =>
  cadenceToggleLocked(c.value.enrolled_count),
)
...
async function changeStatus(status) {
  ...
  await frappeCall('doco_marketing.api.campaigns.set_status', {
...
async function doEnroll() {
  ...
  try {
    const r = await frappeCall('doco_marketing.api.campaigns.enroll_audience', {
...
const TYPE_META = {
  ...
  automation: ['Automatización', '#b9790a', '#fdf6e9'],
}
