...
<template>
  ...
  >
    <!-- toolbar -->
    <div
      class="flex min-h-[52px] flex-none flex-wrap items-center justify-between gap-y-2 border-b border-outline-gray-1 bg-surface-base px-5 py-2"
    >
      <div class="flex flex-wrap items-center gap-3 gap-y-2">
        ...
        <div
          class="flex overflow-hidden rounded-lg border border-outline-gray-2 text-[11.5px] font-semibold"
        >
          <button
            class="px-2.5 py-1"
            :class="
        ...
        <template v-if="view === 'calendar'">
          <div
            class="flex overflow-hidden rounded-lg border border-outline-gray-2 text-[11px] font-semibold"
          >
            <button
              v-for="cv in CAL_VIEWS"
              :key="cv.key"
              class="px-2 py-1"
              :class="
                calView === cv.key
          ...
          <div class="flex items-center gap-1">
            <button
              class="rounded-md px-1.5 py-1 text-ink-gray-6 hover:bg-surface-gray-2"
              @click="shift(-1)"
            >
              ‹
            </button>
            <span
              class="min-w-[140px] text-center text-[13px] font-semibold capitalize text-ink-gray-8"
              >{{ rangeLabel }}</span
            >
            <button
              class="rounded-md px-1.5 py-1 text-ink-gray-6 hover:bg-surface-gray-2"
      ...
      <div class="flex flex-wrap items-center gap-2">
        <router-link
          ...
          :title="__('Publicaciones evergreen reutilizables')"
        >
          🌲 {{ __('Biblioteca') }}
        </router-link>
        <router-link
          to="/social/mentions"
          class="rounded-lg border border-outline-gray-2 px-3 py-1.5 text-[12.5px] font-semibold text-ink-gray-7 hover:bg-surface-gray-2"
          :title="__('Menciones entrantes de clientes')"
        >
          💬 {{ __('Menciones') }}
        </router-link>
        <button
          v-if="isManager"
          class="rounded-lg border border-outline-gray-2 px-3 py-1.5 text-[12.5px] font-semibold text-ink-gray-7 hover:bg-surface-gray-2"
          :title="__('Asignar empleados a sucursales')"
          @click="openShops"
        >
          🏪 {{ __('Sucursales') }}
        </button>
        <button
          v-if="isManager"
          class="rounded-lg border border-outline-gray-2 px-3 py-1.5 text-[12.5px] font-semibold text-ink-gray-7 disabled:opacity-50"
          :disabled="bulkBusy"
          :title="__('Un borrador IA por cada sucursal, desde su inventario')"
          @click="bulkDraft"
        >
        ...
        <button
          ...
          :title="__('Compositor IA: tipo, temporada, brief y ejemplo')"
          @click="openAiComposer"
        >
    ...
    <div
      v-if="
        view === 'calendar' &&
        (visiblePillars.length || visibleStatuses.length || hasFamilies)
      "
      class="flex flex-none flex-wrap items-center gap-1.5 border-b border-outline-gray-1 bg-surface-base px-4 py-2 text-[11px]"
    >
      <button
        v-for="pl in visiblePillars"
        :key="'k' + pl.kind"
        class="rounded-full border px-2 py-0.5 font-medium"
        :class="
          filterKinds.includes(pl.kind)
            ? pl.chip +
              ' border-transparent ring-1 ring-gray-400 dark:ring-gray-500'
            : 'border-outline-gray-2 text-ink-gray-6'
        "
        @click="toggleFilter('kind', pl.kind)"
      >
        {{ pl.emoji }} {{ pl.kind }} · {{ filterCounts.kinds[pl.kind] }}
      </button>
      <span
        v-if="visiblePillars.length && visibleStatuses.length"
      ...
      >
        {{ statusLabel(st) }} · {{ filterCounts.statuses[st] }}
      </button>
      <span v-if="visibleStatuses.length && hasFamilies" class="text-ink-gray-3"
        >|</span
      >
      <button
        v-for="fam in CHANNEL_FAMILIES.filter(
          (f) => filterCounts.families[f.key],
        )"
        :key="'f' + fam.key"
        class="rounded-full border px-2 py-0.5 font-medium"
        :class="
          filterFamilies.includes(fam.key)
            ? 'border-transparent bg-surface-gray-3 text-ink-gray-8 ring-1 ring-gray-400 dark:ring-gray-500'
            : 'border-outline-gray-2 text-ink-gray-6'
        "
        @click="toggleFilter('family', fam.key)"
      >
        {{ fam.emoji }} {{ fam.label }} · {{ filterCounts.families[fam.key] }}
      </button>
      <button
        v-if="activeFilterCount"
        class="ml-auto rounded-full px-2 py-0.5 font-semibold text-ink-gray-5 hover:text-ink-gray-8"
        @click="clearFilters"
      >
        ✕ {{ __('Limpiar') }} ({{ activeFilterCount }})
      </button>
    </div>

    <!-- fetch robustness: a failed load gets a visible banner + retry, never a silently empty grid -->
    <div
      v-if="view === 'calendar' && cal.error"
      class="flex flex-none items-center gap-2 border-b border-outline-gray-1 bg-surface-red-1 px-4 py-2 text-[12px] text-ink-red-8"
      role="alert"
    >
      <span>⚠ {{ __('No se pudo cargar el calendario.') }}</span>
      <button
        class="rounded-md border border-outline-gray-2 bg-surface-base px-2 py-0.5 text-[11px] font-semibold text-ink-gray-7"
        @click="cal.reload()"
      >
        {{ __('Reintentar') }}
      </button>
    ...
    <div
      v-else-if="view === 'calendar' && cal.loading && !cal.data"
      class="flex flex-none items-center gap-1.5 px-4 py-2 text-[12px] text-ink-gray-5"
    >
    ...
    >
      <div
        ...
        :class="
          isMobile
            ? 'h-full min-h-0'
            : 'max-h-[86vh] overflow-hidden rounded-xl'
        "
      >
        <div
        ...
        >
          <span class="text-[14px] font-bold text-ink-gray-9"
            >✨ {{ __('Compositor IA') }}</span
          >
          <!-- brand kit mini-panel: the palette + rules the generator is bound to -->
          <div
            v-if="composeOpts?.brand_kit"
            class="flex items-center gap-1.5 text-[11px] text-ink-gray-6"
          >
            <span
              v-for="c in [
                composeOpts.brand_kit.brand_primary_color,
                composeOpts.brand_kit.brand_secondary_color,
                composeOpts.brand_kit.brand_accent_color,
              ].filter(Boolean)"
              :key="c"
              class="inline-block size-3.5 rounded-full border border-outline-gray-2"
              :style="{ background: c }"
              :title="c"
            />
            <span
              v-if="composeOpts.brand_kit.brand_tagline"
              class="max-w-[180px] truncate italic"
              >«{{ composeOpts.brand_kit.brand_tagline }}»</span
            >
          </div>
        </div>
        <div class="flex-1 space-y-3.5 overflow-y-auto px-4 py-3.5">
          ...
          <div class="flex gap-2">
            <button
              ...
              :class="
                composeMode === 'editorial'
                  ? 'border-blue-500'
                  : 'border-outline-gray-2'
              "
              @click="composeMode = 'editorial'"
            >
              {{ __('Publicación editorial') }}
            </button>
            <button
              class="rounded-lg border px-3 py-2 text-sm"
              :class="
                composeMode === 'catalog'
                  ...
                  : 'border-outline-gray-2'
              "
              @click="composeMode = 'catalog'"
            >
              {{ __('Catálogo / muchas fotos') }}
            </button>
          </div>
          <SocialCatalogGenerator
            v-if="composeMode === 'catalog'"
            :shop="shop"
            :options="composeOpts"
          ...
          <template v-else>
            <div>
              <div
                class="mb-1 text-[11.5px] font-bold uppercase tracking-wide text-ink-gray-5"
              >
                {{ __('Tipo de publicación') }}
              </div>
              <div class="flex flex-wrap gap-1.5">
                <button
                  v-for="k in composeOpts?.kinds || []"
                  :key="k"
                  type="button"
                  class="rounded-full border px-3 py-1 text-[12px] font-semibold"
                  :class="
                    composeForm.post_kind === k
                      ? 'border-transparent text-white'
                      : 'border-outline-gray-2 text-ink-gray-7 hover:bg-surface-gray-2'
                  "
                  :style="
                    composeForm.post_kind === k ? 'background:var(--brand)' : ''
                  "
            ...
            <div>
              ...
              <div class="flex flex-wrap items-center gap-1.5">
                <button
                  v-if="
                    suggestedSeason &&
                    composeForm.season !== suggestedSeason.name
                  "
                  type="button"
                ...
                >
                  {{ suggestedSeason.emoji }} {{ suggestedSeason.name }} ·
                  {{ suggestedSeasonLabel }}
                </button>
                <select
                  v-model="composeForm.season"
                  class="fld rounded-lg border border-outline-gray-2 px-2 py-1 text-[12.5px]"
                >
                  <option :value="null">{{ __('Sin temporada') }}</option>
                  <option
                    v-for="s in composeOpts?.seasons || []"
                    :key="s.name"
                    :value="s.name"
                  >
                    {{ s.emoji }} {{ s.name }}
            ...
            <div
              v-if="
                composeForm.post_kind === 'Producto' &&
                composeOpts?.collections?.length
              "
            >
              <label
                class="mb-1 block text-[11.5px] font-bold uppercase tracking-wide text-ink-gray-5"
                >{{ __('Colección de productos') }}</label
              >
              <select
                v-model="composeForm.collection"
                class="fld w-full rounded-lg border border-outline-gray-2 px-2 py-1.5 text-[12.5px]"
              >
                <option :value="null">
                  {{ __('Elegir un tema coherente del catálogo') }}
                </option>
                <option
                  v-for="c in composeOpts.collections"
                  :key="c.name"
                  :value="c.name"
                >
                  {{ c.title || c.name }}
              ...
              <p class="mt-1 text-[11px] text-ink-gray-5">
                {{
                  __(
                    'Solo productos de esta colección con stock en la sucursal. El brief puede precisar el tema.',
                  )
                }}
              </p>
            </div>
            ...
            <div v-if="(composeOpts?.recent_posts || []).length">
              ...
              <div class="flex gap-2 overflow-x-auto pb-1">
                ...
                >
                  ...
                  <div
                    v-else
                    class="flex h-[64px] w-full items-center justify-center bg-surface-gray-2 text-[18px]"
                  >
                    📝
                  </div>
                  <div
                    class="truncate px-1.5 py-1 text-[10.5px] leading-tight text-ink-gray-7"
                  >
                    {{ p.title || p.caption_head || p.name }}
                  </div>
                </button>
              </div>
            </div>
            ...
            <div>
              ...
              <div
                class="flex flex-wrap gap-x-4 gap-y-1 text-[12.5px] text-ink-gray-8"
              >
                <label class="flex items-center gap-1.5"
                  ><input
                    v-model="composeForm.media_mode"
                    type="radio"
                    value="products"
                  />
                  {{ __('Fotos de productos') }}</label
                >
                <label
                  class="flex items-center gap-1.5"
                  :class="{ 'opacity-40': !composeForm.reference_post }"
                >
                  <input
                    v-model="composeForm.media_mode"
                    type="radio"
                    value="reference"
                    :disabled="!composeForm.reference_post"
                  />
                  {{ __('Reusar las del ejemplo') }}
                </label>
                <label class="flex items-center gap-1.5"
                  ><input
                    v-model="composeForm.media_mode"
                    ...
                    value="none"
                  />
                  {{ __('Sin imagen (la agrego después / Canva)') }}</label
                >
              </div>
            </div>
            <label
              class="flex items-center gap-1.5 text-[12.5px] text-ink-gray-8"
            >
              <input v-model="composeForm.evergreen" type="checkbox" /> 🌲
              {{ __('Marcar como evergreen (rotable si rinde bien)') }}
            </label>
          </template>
        </div>
        <div
        ...
        >
          <button
            ...
            @click="showAiComposer = false"
          >
            {{ __('Cancelar') }}
          </button>
          <button
            v-if="composeMode === 'editorial'"
            class="rounded-lg px-4 py-1.5 text-[12.5px] font-semibold text-white disabled:opacity-50"
            style="background: var(--brand)"
            :disabled="
              composeBusy ||
              (['Noticia', 'Testimonio'].includes(composeForm.post_kind) &&
                !composeForm.brief.trim())
            "
            @click="generateCompose"
          >
            {{ composeBusy ? __('✨ Generando…') : __('✨ Generar borrador') }}
          </button>
        </div>
      </div>
    </component>
    ...
    >
      ...
      >
        ...
        >
          <span class="text-[14px] font-bold text-ink-gray-9">{{
            __('Sucursales y empleados')
          }}</span>
          <button
            class="text-ink-gray-5 hover:text-ink-gray-8"
            @click="showShops = false"
          >
            ✕
          </button>
        </div>
        ...
        >
          ...
          <select
            v-model="onbShop"
            class="fld mb-4 w-full rounded-md border border-outline-gray-2 px-2 py-1.5 text-[13px]"
            @change="onOnbShopChange"
          >
            <option v-for="s in shopOptions" :key="s.name" :value="s.name">
              {{ s.shop_name }}
            </option>
          ...
          <label class="mb-1 block text-[11px] font-semibold text-ink-gray-6">{{
            __('Empleados asignados')
          }}</label>
          <div
            v-if="!(employeesRes.data || []).length"
            class="mb-3 text-[12px] text-ink-gray-4"
          >
            {{
              employeesRes.loading
                ? __('Cargando…')
                : __('Nadie asignado todavía.')
            }}
          </div>
          <div
            v-for="u in employeesRes.data || []"
            :key="u"
            class="mb-1.5 flex items-center justify-between rounded-md bg-surface-gray-1 px-2.5 py-1.5"
          >
            <span class="text-[12.5px] text-ink-gray-8">{{ u }}</span>
            <button
              class="text-[11px] font-semibold text-ink-red-7 hover:underline disabled:opacity-50"
              :disabled="onbBusy"
              @click="unassignEmp(u)"
            >
              {{ __('Quitar') }}
          ...
          <label
            class="mb-1 mt-4 block text-[11px] font-semibold text-ink-gray-6"
            >{{ __('Agregar empleado') }}</label
          >
          <div class="flex items-center gap-2">
            <select
              v-model="newEmp"
              class="fld flex-1 rounded-md border border-outline-gray-2 px-2 py-1.5 text-[13px]"
            >
              <option value="">{{ __('Elegir Marketing User…') }}</option>
              <option v-for="u in availableToAssign" :key="u" :value="u">
                {{ u }}
              </option>
            </select>
            <button
          ...
          <p class="mt-3 text-[11px] text-ink-gray-4">
            {{
              __(
                'Solo aparecen usuarios con rol Marketing User. El gestor (Marketing Manager) ve todas las sucursales.',
              )
            }}
          </p>
        </div>
...
const visiblePillars = computed(() =>
  PILLARS.filter((p) => filterCounts.value.kinds[p.kind]),
)
const visibleStatuses = computed(() =>
  STATUSES.filter((s) => filterCounts.value.statuses[s]),
)
const hasFamilies = computed(
  () => !!(filterCounts.value.families.FB || filterCounts.value.families.IG),
)
...
async function bulkDraft() {
  ...
  try {
    ...
    if ((r.skipped || []).length)
      toast.info(__('Sin stock/IA') + ': ' + r.skipped.join(', '))
    cal.reload()
  } catch (e) {
...
const newEmp = ref('')
const assignableRes = createResource({
  url: 'doco_marketing.api.social.get_assignable_users',
  auto: false,
})
...
const availableToAssign = computed(() =>
  (assignableRes.data || []).filter(
    (u) => !(employeesRes.data || []).includes(u),
  ),
)
function openShops() {
...
async function assignEmp() {
  ...
  onbBusy.value = true
  try {
    await frappeCall('doco_marketing.api.social.assign_employee', {
      user: newEmp.value,
      shop: onbShop.value,
    })
...
async function unassignEmp(user) {
  onbBusy.value = true
  try {
    await frappeCall('doco_marketing.api.social.unassign_employee', {
      user,
      shop: onbShop.value,
    })
...
const suggestedSeason = computed(
  () =>
    composeOpts.value?.current_season ||
    composeOpts.value?.upcoming_season ||
    null,
)
...
const briefPlaceholder = computed(
  () =>
    ({
      Producto: __(
        'Ej: baterías para iPhone; ayudar a consultar compatibilidad, sin prometer instalación',
      ),
      ...
      Noticia: __(
        'Obligatorio: el hecho a anunciar (nuevo horario, servicio, sucursal…)',
      ),
      Testimonio: __(
        'Obligatorio: la historia real del cliente, con sus palabras',
      ),
...
async function openAiComposer() {
  ...
  try {
    composeOpts.value = await frappeCall(
      'doco_marketing.api.social.get_compose_options',
      {
...
async function catalogCreated(result) {
  toast.success(
    `${result.names.length} ${__('borradores de catálogo creados')}`,
  )
  ...
  try {
    ...
    const r = await frappeCall('doco_marketing.api.social.compose_draft', {
      payload: JSON.stringify({
        ...
        brief: f.brief || '',
        collection:
          f.post_kind === 'Producto' ? f.collection || undefined : undefined,
