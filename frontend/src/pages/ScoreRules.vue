...
<template>
  ...
  >
    ...
    <!-- grade legend -->
    <div
      class="flex flex-none items-center gap-3 border-b border-outline-gray-1 px-5 py-2.5"
    >
      <span
        v-for="g in legend"
        :key="g.grade"
        class="flex items-center gap-1.5 text-[11.5px] font-medium text-ink-gray-6"
      >
        <span
          class="rounded px-1.5 py-px text-[10px] font-bold text-white"
          :style="`background:${g.color}`"
          >{{ g.grade }}</span
        >
        {{ g.range }}
      </span>
      <span class="ml-auto text-[11px] text-ink-gray-4"
        >{{ dist.A + dist.B + dist.C + dist.D + dist.Ungraded }}
        {{ __('leads') }}</span
      >
    </div>
    ...
    <div class="flex min-h-0 flex-1">
      ...
      <div class="scb min-h-0 flex-1 overflow-y-auto p-5">
        ...
        <div
          v-if="showAdd"
          class="mb-3 grid grid-cols-[1fr_1fr_120px_1fr_70px_auto] items-center gap-2 rounded-[10px] border border-outline-gray-2 p-3"
        >
          <input
            v-model="form.rule_name"
            :placeholder="__('Nombre')"
            class="fld rounded-md border border-outline-gray-2 px-2 py-1.5 text-[12px] focus:outline-none"
          />
          <input
            v-model="form.field"
            :placeholder="__('Campo (p.ej. source)')"
            class="fld rounded-md border border-outline-gray-2 px-2 py-1.5 text-[12px] focus:outline-none"
          />
          <select
            v-model="form.operator"
            class="fld rounded-md border border-outline-gray-2 px-2 py-1.5 text-[12px]"
          >
          ...
          />
          <button
            class="rounded-md px-2.5 py-1.5 text-[12px] font-semibold text-white disabled:opacity-50"
            style="background: var(--brand)"
            :disabled="!form.rule_name.trim() || !form.field.trim()"
            @click="addRule"
          >
            {{ __('Crear') }}
          </button>
        ...
        >
          ...
          <div class="min-w-0 flex-1">
            ...
            <div class="truncate text-[11.5px] text-ink-gray-5">
              <span class="font-medium">{{ r.field }}</span> {{ r.operator }}
              <span class="font-medium">{{ r.value }}</span>
            </div>
          </div>
          <span
            class="flex-none rounded-md px-2 py-[3px] text-[12px] font-bold"
            :class="
              r.points >= 0
                ? 'text-ink-green-8 bg-surface-green-2'
                : 'text-ink-red-7 dark:text-ink-red-8 bg-surface-red-1'
            "
          >
            {{ r.points >= 0 ? '+' : '' }}{{ r.points }}
          </span>
          <button
            class="flex-none text-[13px] text-ink-gray-4 hover:text-ink-red-7"
            @click="deleteRule(r)"
          >
            ✕
          </button>
      ...
      <div
        class="scb min-h-0 w-[360px] flex-none overflow-y-auto border-l border-outline-gray-1 p-5"
      >
        <div
          class="mb-2.5 text-[11px] font-bold uppercase tracking-[.07em] text-ink-gray-4"
        >
          {{ __('Distribución') }}
        </div>
        <div
        ...
        >
          ...
          <div class="h-2 flex-1 rounded-sm bg-surface-gray-2">
            <div
              ...
              :style="`width:${distPct(g.grade)}%;background:${g.color}`"
            />
          </div>
          <span class="w-8 text-right text-[12px] text-ink-gray-6">{{
            dist[g.grade] || 0
          }}</span>
        </div>
        ...
        <div v-if="preview.length" class="mt-5">
          <div
            class="mb-2.5 text-[11px] font-bold uppercase tracking-[.07em] text-ink-gray-4"
          >
            {{ __('Vista previa') }} ({{ preview.length }})
          </div>
          <div
            v-for="p in preview"
            :key="p.lead"
            class="mb-1.5 flex items-center gap-2 rounded-md border border-outline-gray-1 px-2.5 py-1.5 text-[12px]"
          >
            ...
            <span
              class="font-bold"
              :style="`color:${gradeColor(p.new_grade)}`"
              >{{ p.new_score }}</span
            >
            <span
              class="rounded px-1 text-[9.5px] font-bold text-white"
              :style="`background:${gradeColor(p.new_grade)}`"
              >{{ p.new_grade }}</span
            >
          </div>
        </div>
...
const distRes = createResource({
  url: 'doco_marketing.api.score_rules.get_score_distribution',
  auto: true,
})
const dist = computed(
  () => distRes.data || { A: 0, B: 0, C: 0, D: 0, Ungraded: 0 },
)
function distPct(g) {
  const total =
    ['A', 'B', 'C', 'D', 'Ungraded'].reduce(
      (a, k) => a + (dist.value[k] || 0),
      0,
    ) || 1
...
const previewRes = createResource({
  url: 'doco_marketing.api.score_rules.preview_scores',
})
