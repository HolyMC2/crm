  ...
  >
    ...
    <div class="p-5">
      <div class="grid grid-cols-5 gap-3">
        ...
        <Kpi
          :label="__('Ingresos vía CRM')"
          :value="money(kpis.revenue_via_crm)"
          color="var(--brand)"
        />
        <Kpi
          :label="__('Ticket promedio')"
          :value="money(kpis.avg_order_value)"
        />
        <Kpi
          :label="__('Carritos abandonados')"
          :value="kpis.cart_abandoned ?? '—'"
        />
      ...
      >
        ...
        <div
          v-for="(l, i) in logs"
          :key="i"
          class="border-b border-outline-gray-1 py-2 text-[12px] text-ink-gray-7"
        >
          {{ l.event || l }}
        </div>
      </div>
...
const kpisRes = createResource({
  url: 'doco_marketing.api.webshop.get_webshop_kpis',
  auto: true,
})
const logsRes = createResource({
  url: 'doco_marketing.api.webshop.get_webhook_logs',
  auto: true,
})
