...
test.describe('espresso v2 shared layer', () => {
  test('avatar palette resolves to real ink tokens in both themes', async ({
    page,
  }) => {
    await login(page)
    ...
    console.log(
      'inline-styled palette elements:',
      JSON.stringify(swatches, null, 2),
    )

    await page.screenshot({
      path: path.join(OUT, 'leads-light.png'),
      fullPage: false,
    })
    ...
    for (const v of inks)
      expect(darkInks[v], `${v} must resolve in dark`).not.toBe('')
    await page.screenshot({
      path: path.join(OUT, 'leads-dark.png'),
      fullPage: false,
    })
  })
  ...
  test('mobile sidebar active item has a real background', async ({ page }) => {
    ...
    expect(
      await cssVar(page, '--surface-elevation-3'),
      'elevation-3 must resolve',
    ).not.toBe('')
    expect(
      await cssVar(page, '--surface-selected'),
      'retired token must be gone',
    ).toBe('')

    // Open the drawer so MobileSidebar (SidebarLink's only importer) is mounted.
    await page
      .locator('button:has(svg), [aria-label*="menu" i]')
      .first()
      .click()
    ...
    const probe = await page.evaluate(() => {
      ...
      return {
        migrated: mk('bg-surface-elevation-3'),
        retired: mk('bg-surface-selected'),
      }
    })
    console.log('SidebarLink active-state contract:', JSON.stringify(probe))
    expect(probe.migrated, 'bg-surface-elevation-3 must paint').not.toMatch(
      /rgba\(0, 0, 0, 0\)|transparent/,
    )
    expect(
      probe.retired,
      'bg-surface-selected must paint NOTHING (retired)',
    ).toMatch(/rgba\(0, 0, 0, 0\)|transparent/)

    await page.screenshot({
      path: path.join(OUT, 'mobile-light.png'),
      fullPage: false,
    })
    await setTheme(page, 'dark')
    await page.waitForTimeout(1200)
