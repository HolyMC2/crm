...
const SITES = [
  [
    'RepairOrdersSection:125 tracker badge',
    'text-ink-green-1',
    'bg-surface-green-7',
  ],
  ...
  [
    'PILLAR Servicio / RepairOrders:190 / CampaignDetail:15',
    'text-ink-violet-8',
    'bg-surface-violet-2',
  ],
  [
    'PILLAR Producto',
    'text-ink-blue-7 dark:text-ink-blue-8',
    'bg-surface-blue-2',
  ],
  ['PILLAR Noticia', 'text-ink-red-6 dark:text-ink-red-8', 'bg-surface-red-2'],
  [
    'PILLAR Testimonio + all 23 green-2 chips',
    'text-ink-green-8',
    'bg-surface-green-2',
  ],
  [
    'red-1 chips (WorkloadView:75, SocialComposer:41/153, ScoreRules:73)',
    'text-ink-red-7 dark:text-ink-red-8',
    'bg-surface-red-1',
  ],
  [
    'KEEP: inverting gray chip (SocialCalendar x3, SocialMentions:49)',
    'text-ink-base',
    'bg-surface-gray-10',
  ],
]
...
test('W2-doco: every edited class pair, both themes', async ({ page }) => {
  ...
  for (const [label, fg, bg] of SITES) {
    ...
    rows.push({
      ...
      fg,
      bg,
      light: r.light.contrast,
      dark: r.dark.contrast,
      colorDark: r.dark.color,
    })
  }
  ...
  for (const r of rows) {
    ...
    console.log(
      `${fmt(r.light)} L ${fmt(r.dark)} D  ${flag}  ${r.label}\n            ${r.fg}  on  ${r.bg}`,
    )
  }
  ...
  for (const r of rows.slice(0, 5)) {
    ...
    const ch = r.colorDark
      .match(/[\d.]+/g)
      .map(Number)
      .slice(0, 3)
    const norm = r.colorDark.startsWith('color(') ? ch : ch.map((v) => v / 255)
    for (const c of norm)
      expect(c, `${r.label} must be pure white in dark`).toBeGreaterThan(0.99)
