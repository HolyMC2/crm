...
describe('repair ready quick action', () => {
  it('uses Listo para Entregar when the canonical stage exists', () => {
    expect(repairReadyStage([stage('Por Entregar'), stage('Listo para Entregar')]).name).toBe('Listo para Entregar')
  })
  ...
  it('respects the visible tenant language and excludes legacy aliases', () => {
    expect(repairReadyStage([stage('Listo para Entregar', 1), stage('Por Entregar', 1), stage('Ready for Pickup', '0')]).name).toBe('Ready for Pickup')
  })
  it('supports mail-in workshops', () => {
    expect(repairReadyStage([stage('Ready to Ship')]).name).toBe('Ready to Ship')
  })
  it('does not select a missing, hidden or terminal stage', () => {
    expect(repairReadyStage([stage('Por Entregar', 1), stage('Listo para Entregar', 0, 'Won')])).toBeNull()
