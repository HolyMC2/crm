...
describe('follow-up work queues', () => {
  it('distinguishes a missing task from a task awaiting a date', () => {
    expect(
      followUpFilters('missing', '2026-09-13', ['Ganado', 'Lost']),
    ).toEqual([
      ['next_activity_task', 'is', 'not set'],
      ['status', 'not in', ['Ganado', 'Lost']],
    ])
    expect(followUpFilters('undated')).toEqual([
      ['next_activity_task', 'is', 'set'],
      ['next_activity_at', 'is', 'not set'],
    ])
  })
  it('uses the supplied site day for disjoint overdue and today boundaries', () => {
    expect(followUpFilters('overdue', '2026-12-31')).toContainEqual([
      'next_activity_at',
      '<',
      '2026-12-31 00:00:00',
    ])
    expect(followUpFilters('today', '2026-12-31')).toContainEqual([
      'next_activity_at',
      'between',
      ['2026-12-31 00:00:00', '2026-12-31 23:59:59.999999'],
    ])
  })
  it('leaves all deals unconstrained and accepts a tenant with no closed statuses', () => {
    expect(followUpFilters('all')).toEqual([])
    expect(followUpFilters('missing')).toEqual([
      ['next_activity_task', 'is', 'not set'],
    ])
  })
})
...
describe('follow-up due date', () => {
  ...
  it('splits a stored due date back into the fields the popover edits', () => {
    expect(followUpParts('2026-09-16 10:30:00')).toEqual({
      date: '2026-09-16',
      time: '10:30',
    })
    // midnight is the "no hour" marker, not a choice to show back
    expect(followUpParts('2026-09-16 00:00:00')).toEqual({
      date: '2026-09-16',
      time: '',
    })
...
describe('the canonical follow-up task', () => {
  ...
  it('never writes next_activity_* — those are derived from the task', () => {
    ...
    expect(Object.keys(doc).some((k) => k.startsWith('next_activity'))).toBe(
      false,
    )
  })
  it('leaves an unowned deal unassigned instead of assigning an empty user', () => {
    expect('assigned_to' in followUpTaskDoc({ ...draft, owner: '' })).toBe(
      false,
    )
  })
  it('defaults the activity type and drops an undated due date', () => {
    const doc = followUpTaskDoc({
      deal: 'D',
      title: 'Llamar',
      date: '',
      type: '',
...
describe('follow-up draft validation', () => {
  it('names the rule that is broken so the caller owns the wording', () => {
    ...
    expect(followUpDraftError({ title: '   ', date: '2026-09-16' })).toBe(
      'title',
    )
    ...
    expect(
      followUpDraftError({ title: 'Llamar', date: '2026-09-16', time: '' }),
    ).toBe('')
  })
})
