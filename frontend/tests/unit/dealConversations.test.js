...
async function mount(extra = {}) {
  ...
  const app = createApp(DealConversations, {
    doctype: 'CRM Deal',
    name: 'DEAL-1',
    ...extra,
  })
...
const linked = {
  ...
  provider: 'WhatsApp',
  account_id: '123',
  peer_id: '5215550198766',
  display_name: 'Pablo',
  materialized: true,
  control_state: 'Human',
  preview: 'Muy bien gracias',
  last_message_at: '2026-09-09 16:17:59',
}
const implied = {
  name: null,
  provider: 'WhatsApp',
...
describe('deal native conversations', () => {
  it('opens the exact conversation in the conversation space and keeps unrelated deal query parameters out', async () => {
    ...
    expect(call).toHaveBeenCalledWith(
      ...
      { doctype: 'CRM Deal', name: 'DEAL-1', cursor: null },
    )
    expect(el.querySelector('li a').getAttribute('href')).toBe(
      '/inbox?workspace=conversations&conversation=exact-thread',
    )
  ...
  it('materializes a thread implied by the record messages through the explicit open, then navigates to it', async () => {
    call.mockImplementation(async (method) => {
      if (method.endsWith('list_for_reference'))
        return { items: [implied], next_cursor: null }
    ...
    await vi.waitFor(() =>
      expect(call).toHaveBeenCalledWith(
        ...
        { provider: 'WhatsApp', account_id: '123', peer_id: '5215550198766' },
      ),
    )
    await vi.waitFor(() =>
      expect(router.currentRoute.value.query).toEqual({
        workspace: 'conversations',
        conversation: 'opened-thread',
      }),
    )
  })
  ...
  it('compact line above the deal thread links each thread and stays hidden when there are none', async () => {
    ...
    expect(el.querySelector('nav a').getAttribute('href')).toBe(
      '/inbox?workspace=conversations&conversation=exact-thread',
    )
    ...
    expect(empty.el.textContent).not.toContain(
