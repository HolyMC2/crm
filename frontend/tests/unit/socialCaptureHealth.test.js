...
} from '../../src/utils/socialCaptureHealth'

describe('social capture evidence', () => {
  it('does not turn an existing subscription into a delivery guarantee', () => {
    expect(
      captureHealthLabel('subscription_present_delivery_unverified'),
    ).toContain('recepción sin comprobar')
    expect(captureHealthLabel('surprise_state')).toBe('Conexión sin verificar')
    expect(captureHealthLabel('missing_mention_subscription')).toContain(
      'Falta',
    )
  })
