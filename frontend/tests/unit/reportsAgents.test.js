...
describe('sortAgents', () => {
  ...
  it('numeric desc (won_value)', () => {
    expect(
      sortAgents(rows, 'won_value', 'desc').map((r) => r.agent_name),
    ).toEqual(['Ana', 'Beto', 'Caro'])
  })
  it('numeric asc (open)', () => {
    expect(sortAgents(rows, 'open', 'asc').map((r) => r.agent_name)).toEqual([
      'Ana',
      'Caro',
      'Beto',
    ])
  ...
  it('null median sinks to the bottom in BOTH directions', () => {
    expect(
      sortAgents(rows, 'median_response_secs', 'asc').map((r) => r.agent_name),
    ).toEqual(['Caro', 'Beto', 'Ana'])
    expect(
      sortAgents(rows, 'median_response_secs', 'desc').map((r) => r.agent_name),
    ).toEqual(['Beto', 'Caro', 'Ana'])
  })
  it('text sort uses es locale (asc)', () => {
    expect(
      sortAgents(rows, 'agent_name', 'asc').map((r) => r.agent_name),
    ).toEqual(['Ana', 'Beto', 'Caro'])
  })
...
describe('nextSort', () => {
  it('a new column starts descending', () => {
    expect(nextSort({ key: 'won', dir: 'asc' }, 'open')).toEqual({
      key: 'open',
      dir: 'desc',
    })
  })
  it('the active column toggles direction', () => {
    expect(nextSort({ key: 'won', dir: 'desc' }, 'won')).toEqual({
      key: 'won',
      dir: 'asc',
    })
    expect(nextSort({ key: 'won', dir: 'asc' }, 'won')).toEqual({
      key: 'won',
      dir: 'desc',
    })
  })
