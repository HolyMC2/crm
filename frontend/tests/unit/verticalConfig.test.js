...
const h = vi.hoisted(() => ({ available: true, calls: [], behavior: async () => ({ schemaVersion: 1, providers: [] }) }))
vi.mock('@/utils/crmCapabilities', () => ({ hasApp: () => h.available }))
vi.mock('frappe-ui', () => ({ call: (...args) => { h.calls.push(args); return h.behavior(...args) } }))
import { useVerticalConfig } from '@/composables/verticalConfig'

const provider = (app) => ({
  id: `${app}:workspace`, app, schemaVersion: 1, label: app, modules: [],
  slots: [{ id: 'workspace', slot: 'data_tab', component: 'ProviderWorkspace', route: `/${app}` }],
})
...
describe('vertical discovery lifecycle', () => {
  ...
  beforeEach(() => {
    h.calls = []
    h.available = true
    h.behavior = async () => ({ schemaVersion: 1, providers: [provider('clinica')] })
    scope = effectScope()
  })
  ...
  it('calls CRM-owned endpoint with only the authorized record context', async () => {
    const config = scope.run(() => useVerticalConfig(() => 'deal-1'))
    await flush()
    expect(h.calls).toEqual([['crm.api.capabilities.get_vertical_config', {
  ...
  it('clears personalized cards on focus and keeps late permission replies out', async () => {
    const config = scope.run(() => useVerticalConfig(() => 'deal-1'))
    await flush()
    const finish = []
    h.behavior = () => new Promise((resolve) => finish.push(resolve))
  ...
  it('contains a failed lookup, refreshes a continuously open view and cleans up', async () => {
    ...
    const config = scope.run(() => useVerticalConfig(() => 'deal-1'))
    await flush()
    h.behavior = async () => { throw new Error('provider unavailable') }
  ...
  it('preserves a same-record legacy draft on transport errors but clears it on access denial', async () => {
    const sections = [{ section_key: 'repair', vue_component: 'RepairOrdersSection' }]
    h.behavior = async () => ({ schemaVersion: 1, sections, providers: [provider('clinica')] })
    const config = scope.run(() => useVerticalConfig(() => 'deal-1'))
    await flush()
    h.behavior = async () => { throw new Error('Network disconnected') }
    window.dispatchEvent(new Event('focus'))
    await flush()
    expect(config.value.sections).toEqual(sections)
    expect(config.value.providers).toEqual([])
    h.behavior = async () => { throw Object.assign(new Error('Denied'), { status: 403 }) }
    window.dispatchEvent(new Event('focus'))
    await flush()
    expect(config.value).toBeNull()
  })
...
describe('compiled vertical contributions', () => {
  it('renders both authorized providers without a selected vertical', () => {
    const result = resolveVerticalSections({ schemaVersion: 1, providers: [provider('clinica'), provider('taller')] }, 'data_tab', () => true)
  ...
  it('filters unsupported routes/components and missing app dependencies', () => {
    ...
    const result = resolveVerticalSections({ schemaVersion: 1, providers: [bad, provider('taller')], sections: [
      { enabled: 1, render_in: 'data_tab', vue_component: 'DealDocumentsSection' },
      { enabled: 1, render_in: 'data_tab', vue_component: 'ArbitraryComponent' },
    ] }, 'data_tab', (app) => app === 'clinica')
    expect(result).toEqual([])
  })

  it('preserves existing repair layout order and hides marketing documents without marketing', () => {
    const result = resolveVerticalSections({ schemaVersion: 1, sections: [
      { section_key: 'docs', enabled: 1, render_in: 'data_tab', vue_component: 'DealDocumentsSection', idx: 1 },
      { section_key: 'repair', enabled: 1, render_in: 'data_tab', vue_component: 'RepairOrdersSection', idx: 2 },
    ] }, 'data_tab', (app) => ['doco', 'taller', 'erpnext'].includes(app))
