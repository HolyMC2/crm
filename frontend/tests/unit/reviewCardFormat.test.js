...
describe('review card links', () => {
  it('routes deals to Deal 360 and leads to the lead page', () => {
    expect(recordRoute('CRM Deal', 'CRM-DEAL-2026-00407')).toEqual({
      name: 'Deal 360', params: { dealId: 'CRM-DEAL-2026-00407' },
    })
    expect(recordRoute('CRM Lead', 'CRM-LEAD-1')).toEqual({ name: 'Lead', params: { leadId: 'CRM-LEAD-1' } })
  ...
  it('falls back to the contact behind the number when the reference has no page', () => {
    expect(recordRoute('Repair Order', 'RO-00255', 'Pablo Hernández')).toEqual({ name: 'Contact', params: { contactId: 'Pablo Hernández' } })
    expect(recordRoute('CRM Deal', 'CRM-DEAL-1', 'Pablo')).toEqual({ name: 'Deal 360', params: { dealId: 'CRM-DEAL-1' } })
  })
  it('builds Desk links with the slug convention and encodes the name', () => {
    expect(deskHref('Repair Order', 'RO-00255')).toBe('/app/repair-order/RO-00255')
...
describe('review card identity line', () => {
  it('joins repair type, device, title and organization without repeats', () => {
    expect(aboutLine({ repair_type: 'Cambio de pantalla', device: 'iPhone 12', title: 'cambio de pantalla' }))
      .toBe('Cambio de pantalla · iPhone 12')
    expect(aboutLine({ device: ' ', title: 'Reparación', organization: 'Acme' })).toBe('Reparación · Acme')
    expect(aboutLine({})).toBe('')
  })
  it('drops a deal title that only restates the phone or the device — customer pair', () => {
    expect(aboutLine({ repair_type: 'twip test', device: 'TWIP DEV', title: '+5215555550000', customer_phone: '+5215555550000' }))
      .toBe('twip test · TWIP DEV')
    expect(aboutLine({
      repair_type: 'Quitar Virus', device: 'Realme C63', title: 'Realme C63 — Test Orders', customer_name: 'Test Orders',
    })).toBe('Quitar Virus · Realme C63')
    expect(aboutLine({ device: 'Realme C63', title: 'Pantalla rota, urge', customer_name: 'Ana' })).toBe('Realme C63 · Pantalla rota, urge')
  })
})
