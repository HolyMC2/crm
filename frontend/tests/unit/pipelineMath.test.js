...
describe('displayValue', () => {
  it('prefers the expected value when it is set', () => {
    expect(displayValue({ expected_deal_value: 1200, deal_value: 300 })).toBe(1200)
  })
  ...
  it('falls back to the invoiced value when expected is missing, zero or blank', () => {
    ...
    expect(displayValue({ expected_deal_value: null, deal_value: 300 })).toBe(300)
...
describe('weightedTotal', () => {
  ...
  it('ignores stages with no probability rather than counting them in full', () => {
    ...
    expect(weightedTotal(counts, [{ value: 'Abandonado', probability: null }])).toBe(0)
  ...
  it('ignores stages with no loaded aggregate, and hidden stages absent from the list', () => {
    const counts = { Aprobado: { count: 1, deal_value: 500 }, Approved: { count: 9, deal_value: 9000 } }
  ...
  it('accepts status rows keyed by name as well as value', () => {
    ...
    expect(weightedTotal(counts, [{ name: 'Aprobado', probability: 50 }])).toBe(100)
  })
